...
| **Bestehende Gegenmaßnahmen** | `WritePort.precheck()` vor `run()`; Permit mit Ablauf; Stufenfolge-Prüfung; **44 formale Write-Safety-Vektoren** (`npm run formal:conform` → 44/44); `SafetyManager`-Policy je Operation |
| **Restrisiko** | Die Freigabe schützt vor dem *falschen Zeitpunkt*, nicht vor dem *falschen Aufrufer* |
| S / E / C | ___ / ___ / ___ |
| **ASIL** | **offen** |
| **Maßnahme** | Authentifizierung der API (siehe [`iso-21434-cybersecurity.md`](iso-21434-cybersecurity.md)) |
...
| **Bestehende Gegenmaßnahmen** | `WARN`-Zeile beim Start; Sicherheits-Header (CSP mit `frame-ancestors 'none'`, `x-frame-options: DENY`, `cross-origin-resource-policy: same-origin`); `x-content-type-options: nosniff` |
| **Lücke messbar** | `grep -c "authoriz\|bearer\|token" apps/web/src/server.ts` → **0**; kein `strict-transport-security` in `apps/web/src/static-assets.ts` |
| S / E / C | ___ / ___ / ___ |
| **ASIL** | **offen** |
