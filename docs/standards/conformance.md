...
| **ISO 21434 / UNECE R155** (Cybersecurity) | **fehlt** | TARA, CSMS, Schwachstellenprozess; **konkret: der Workbench-Server hat keine Authentifizierung** (`grep -c "authoriz\|bearer\|token" apps/web/src/server.ts` → **0**) | Entwicklung + CSO | Code-Teil in diesem Repo machbar |
