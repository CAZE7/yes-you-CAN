...
| 16. Geführte Diagnose | Prüfabläufe als Daten (Symptom → Hypothesen → Messschritt → Auswertung), Verbraucher von Schritt 15 | 🟡 **die Bewertungsseite ist da** (ADR 0038): `core/src/evidence/hypotheses.ts` bewertet `checks[]` gegen die Messwerte im `windowMs`-Fenster (IR-`summariseWindow`), jedes Muster bekommt `confirmed`/`refuted`/`untested`, `nextTest` ist der erste unentschiedene Check, `confidenceOf` eine nachrechenbare Regel mit Deckel, und `runtime.evidence.hypotheses()` liefert sie der Workbench (Empfehlung „next test for …“). `patterns[]` sind die Hypothesen, `checks[]` die Messschritte, Pattern-IDs sind je Fahrzeug eindeutig und damit adressierbar. **Offen** bleibt der interaktive Ablauf (Messung aus dem UI anstoßen, Fenster schließen, nächsten Schritt wählen), was „nicht prüfbar“ für einen Ablauf bedeutet, und wie eine Beziehung zwischen zwei Messpunkten ausgedrückt wird — heute stehen für „linke gegen rechte Radgeschwindigkeit“ zwei Checks mit demselben Fenster nebeneinander und die Beziehung im `expect`-Text (ADR 0025) | `packages/definitions`, `packages/runtime`, `apps/web` |
