...
| 16–18 | Fault Injection · Chaos Testing · Differential Testing | 🟡 **Fault-Injection am Transport-Seam erledigt 2026-09-14** (ADR 0039): `tools/simulators/src/faulty-link.ts` (Antwort schlucken, Request nicht durchlassen, falsche Service-ID, Kürzung, Müll, hängender Endrahmen, 0x78-Dauerschleife, ungeordnetes Frame, Antwort nach dem Timeout) zwischen **realer** `UdsServer`-ECU und `UdsClient`, 14 Fälle in `tests/protocol/fault-injection.test.ts`; dabei ein echtes Loch gefunden und gestopft — eine abgeschnittene 0x19-Antwort las sich als „kein Fehler gespeichert“ | Chaos über mehrere ECUs/Transporte und Differential gegen einen zweiten Stack (#17/#18): der Simulator kann weiterhin Zustände, keine Fehlerklassen-Szenarien über mehrere Zyklen |
