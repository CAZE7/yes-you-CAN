...
const testOem: OemProtocol = {
  ...
  interpretDtc: (code) =>
    code === "P0420"
      ? { code, description: "Manufacturer hint: check lambda sensors first", severity: "major" }
      : undefined,
};

async function withEngine(
  options: {
    definitions?: readonly (typeof genericPackage)[];
...
test("an ECU no definition package knows is named by the OEM hook", async () => {
  await withEngine({ definitions: [], oemProtocols: [testOem] }, async (engine) => {
    ...
    assert.equal(
      record.name,
      "Engine (from OEM hook)",
      "the OEM role must name an otherwise unknown ECU",
    );
  });
});
...
test("the engine exposes which OEM modules are registered, with provenance", async () => {
  await withEngine({ definitions: [genericPackage], oemProtocols: [testOem] }, async (engine) => {
    ...
    assert.match(
      listed[0]?.provenance ?? "",
      /example-placeholder/,
      "provenance must be visible (AGENTS 24)",
    );
  });
});
