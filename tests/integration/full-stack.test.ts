...
test("ECU discovery finds the ECUs defined by the package (AGENTS 12)", async () => {
  ...
  assert.ok(
    ...
    `engine ECU must respond, got ${rxIds.map((id) => id.toString(16))}`,
  );
...
test("VIN is detected automatically and validated against ISO 3779 (AGENTS 11)", async () => {
  ...
  assert.equal(
    analysis.checkDigit,
    "valid",
    `check digit must validate: ${analysis.notes.join("; ")}`,
  );
...
test("a freeze frame is decoded through the definition package, byte by byte (AGENTS 13, 20)", async () => {
  ...
  assert.equal(
    frame.documented,
    true,
    `layout must be fully documented, notes: ${(frame as { notes?: string[] }).notes?.join("; ") ?? ""}`,
  );
  assert.deepEqual(
...
test("the engine tracks when a fault code was first and last seen (AGENTS 20)", async () => {
  ...
  assert.ok(
    catalyst.lastSeen && catalyst.lastSeen >= catalyst.firstSeen,
  ...
  assert.ok(
    related.some((entry) => entry.id === "engine.long_term_fuel_trim"),
    "related signals come from the definition package",
  );
});
...
test("live DIDs are read and decoded into physical values (AGENTS 14)", async () => {
  ...
  assert.equal(coolant.value, (Number.parseInt(coolant.rawHex.slice(0, 2), 16) - 40) * 1);
});
...
test("several DIDs are polled in parallel across ECUs but serialised per ECU (AGENTS 15)", async () => {
  ...
  assert.ok(
    stats.samples >= 9,
    `expected at least 9 samples (3 rounds × 3 engine signals), got ${stats.samples}`,
  );
...
test("recorded samples share one time axis so charts can be synchronised (AGENTS 16)", () => {
  ...
  for (let i = 1; i < merged.length; i++) {
    assert.ok(
      (merged[i] as { t: number }).t >= (merged[i - 1] as { t: number }).t,
      "merged samples must stay time ordered",
    );
  }
});
...
test("CSV and JSON exports contain raw and decoded values (AGENTS 17)", () => {
  ...
  assert.ok(
    lines.some((line) => line.includes("engine.rpm")),
    "rpm samples must be exported",
  );
  assert.ok(
    lines.some((line) => line.includes("engine.coolant_temperature")),
    "coolant samples must be exported",
  );
...
test("write operations are refused in the default session (read-only first, AGENTS 11)", async () => {
  ...
  await assert.rejects(
    handle.session.client.writeDataByIdentifier(0x2001, fromHex("00 01")),
    /serviceNotSupportedInActiveSession|requestOutOfRange|conditionsNotCorrect/,
  );
});

async function waitForLive(
  engine: {
    statistics: { rounds: number; samples: number; errors: number; averageRoundMs: number };
    isRunning: boolean;
  },
  timeoutMs = 4000,
): Promise<{ rounds: number; samples: number; errors: number; averageRoundMs: number }> {
