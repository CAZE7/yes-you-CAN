...
test.skipIf(!hasSocat())('a serial stream reads and writes over a real character device', async () => {
...
test.skipIf(!hasSocat())('line settings are applied with stty, and a bad device fails loudly', async () => {
  const pair = await createPtyPair();
  try {
...
test.skipIf(!hasSocat())('the slcan adapter drives a real serial port and parses frames coming back', async () => {
  const pair = await createPtyPair();
  const device = createDeviceSide(pair.b);
