...
async function recordSession(): Promise<Recording> {
  ...
  const values = snapshot
    .filter(
      (signal): signal is NonNullable<typeof signal> => signal !== null && signal !== undefined,
    )
...
test("a recorded session contains both directions of the conversation", async () => {
  ...
  assert.ok(
    recording.traceFrames > 10,
    `expected a substantial trace, got ${recording.traceFrames} frames`,
  );
...
test("a non-session export is rejected instead of replaying garbage", () => {
  assert.throws(
    () => recordingFromSessionJson(JSON.stringify({ format: "something-else", trace: [] })),
    /not a vdp.session export/,
  );
});
...
test("the same requests replayed from the recording yield the same responses", async () => {
  ...
  const connection = new IsoTpConnection(
    bus,
    { txId: 0x7e0, rxId: 0x7e8, channel: "vcan0" },
    logger,
  );
  connection.open();
  ...
  assert.ok(
    ...
    "the serial number DID must be answered from the recording",
  );

  assert.equal(
    bus.deviations.length,
    0,
    `replay must not deviate: ${JSON.stringify(bus.deviations)}`,
  );
...
test("replayed values decode identically to the live run", async () => {
  ...
  const signal = genericPackage.signals.find(
    (candidate) => candidate.id === "engine.coolant_temperature",
  );
...
test("a request on an identifier the recording never used is reported", async () => {
  const recording = await recordSession();
  const replayRecording = recordingFromSessionJson(recording.json);
  const bus = new ReplayTransport(replayRecording, {
    logger,
    immediate: true,
    matchByIdOnly: false,
  });
  ...
  const connection = new IsoTpConnection(
    bus,
    { txId: 0x6ff, rxId: 0x6fe, channel: "vcan0" },
    logger,
  );
  connection.open();
...
test("a changed request payload is reported when matching strictly", async () => {
  ...
  assert.ok(
    payloadDiffers.length > 0,
    `expected a payload deviation, got ${JSON.stringify(bus.deviations)}`,
  );
  assert.ok(
    payloadDiffers[0]?.recordedPayload,
    "the deviation must name the recorded payload for comparison",
  );
  await bus.close();
});
...
test("replay delivers frames to subscribers with direction rx", async () => {
  ...
  bus.subscribe((frame) =>
    seen.push({ id: frame.id, ...(frame.direction ? { direction: frame.direction } : {}) }),
  );
  ...
  assert.ok(
    seen.every((frame) => frame.direction === "rx"),
