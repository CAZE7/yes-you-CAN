...
function createPair(channel: string, overrides: Record<string, unknown> = {}): Pair {
  ...
  const connection = new IsoTpConnection(
    testerBus,
    { txId: 0x7e0, rxId: 0x7e8, channel, ...overrides },
    logger,
  );
  connection.open();
...
test("block size limits how many consecutive frames the tester accepts at once", async () => {
  ...
  await pair.waitFor(2);
  assert.equal(
    flowStatus(pair.received[1]),
    3,
    "the first frame must be answered with flow control",
  );
  ...
  assert.equal(
    flowStatus(pair.received[2]),
    3,
    "a second flow control must follow the exhausted block",
  );
  ...
  assert.equal(
    toHex(response),
    "62 F1 90 41 42 43 44 45 46 47 48 49 4A 4B 4C 4D 4E 4F 50 51 52 53 54 55 56 57 58",
  );
});
...
test("a Wait flow control from the tester does not abort the transfer", async () => {
  ...
  assert.ok(
    ...
    `expected the remaining frames to be sent, got ${consecutive.length}`,
  );
});
...
test("a payload longer than 4095 bytes uses the escape sequence in the first frame", async () => {
  ...
  assert.equal(
    firstFrame[0] ?? 0,
    0x10,
    "the low nibble must be zero to signal the escape sequence",
  );
  const announced =
    ((firstFrame[2] ?? 0) << 24) |
    ((firstFrame[3] ?? 0) << 16) |
...
test("sequence numbers wrap from 0xF back to 0x0 across a long transfer", async () => {
  ...
  const sequences = pair.received
    .filter((frame) => flowStatus(frame) === 2)
    .map((frame) => (frame.payload[0] ?? 0) & 0x0f);
