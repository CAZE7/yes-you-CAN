...
function walk(dir, depth = 0) {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (entry === 'dist') { rmSync(full, { recursive: true, force: true }); removed.push(full); continue; }
    if (depth < 4) walk(full, depth + 1);
  }
}

for (const r of roots) walk(r);
for (const f of readdirSync('.').filter((f) => f.endsWith('.tsbuildinfo'))) { rmSync(f); removed.push(f); }
