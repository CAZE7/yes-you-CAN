...
async function json(base: string, path: string, init?: RequestInit): Promise<{ status: number; body: unknown; type: string }> {
  const response = await fetch(`${base}${path}`, init);
  const type = response.headers.get('content-type') ?? '';
  return { status: response.status, body: type.includes('json') ? await response.json() : await response.text(), type };
}
...
test('start connects to the simulated vehicle and discovers ECUs', async () => {
  await withServer(async (base, server) => {
    ...
    const ecus = (state.ecus as Array<{ name: string; identification: Array<{ label: string; value: string }> }>);
    const engine = ecus.find((ecu) => ecu.name.includes('Engine'));
    assert.ok(engine, 'the engine ECU must be discovered');
    assert.ok(engine.identification.some((entry) => entry.label === 'VIN' && entry.value === '1HGCM82633A004352'));
    void server;
  });
});
...
test('the DTC scan returns the seeded fault codes with severity', async () => {
  await withServer(async (base) => {
    ...
    const dtcs = (scanned.body as { dtcs: Array<{ code: string; severity: string; ecu: string }> }).dtcs;
...
test('live data produces decoded samples with statistics', async () => {
  await withServer(async (base) => {
    await json(base, '/api/start', { method: 'POST' });
    const started = await json(base, '/api/live/start', { method: 'POST', body: JSON.stringify({ signalIds: ['engine.rpm'] }) });
    ...
    assert.equal(state.samples[0]?.name, 'Engine speed', 'the snapshot must carry the definition name, not the id');
    assert.match(state.samples[0]?.rawHex ?? '', /^[0-9A-F]{2}( [0-9A-F]{2})*$/, 'raw bytes stay visible (AGENTS 18)');
...
test('exports produce CSV, JSON, HTML and a valid PDF', async () => {
  await withServer(async (base) => {
    ...
    const session = JSON.parse(await (await fetch(`${base}/api/export/session.json`)).text()) as { format: string; dtcs: unknown[] };
...
test('analysis returns a labelled result from the local provider', async () => {
  await withServer(async (base) => {
    ...
    const analysis = result.body as { provider: string; source: string; findings: unknown[]; recommendations: string[]; summary: string };
