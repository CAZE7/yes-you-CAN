...
async function json(base: string, path: string, init?: RequestInit): Promise<{ status: number; body: unknown }> {
...
test('the adapter list reports availability without opening a device', async () => {
  await withServer(async (base, server) => {
    ...
    const body = listed.body as {
      mode: string;
      selected: { id: string };
      adapters: Array<{ id: string; probe: { available: boolean; detail: string }; capabilities: { can: boolean } }>;
    };
...
test('an unknown adapter id is rejected with 400 and a list of valid ids', async () => {
  await withServer(async (base, server) => {
    ...
    assert.equal(result.status, 400, 'a configuration mistake is the caller\'s error, not a server fault');
    assert.match(String((result.body as { error: string }).error), /unknown adapter/);
    assert.equal(server.backend.adapterSelection.id, 'simulator', 'a rejected selection must not take effect');
  });
});
...
test('selecting an adapter switches the transport mode and probes it', async () => {
  await withServer(async (base, server) => {
    ...
    const body = result.body as { adapter: { probe: { available: boolean; hints?: string[] } }; reconnectRequired: boolean };
    assert.equal(body.reconnectRequired, false);
    assert.equal(body.adapter.probe.available, false);
    assert.ok((body.adapter.probe.hints ?? []).length > 0, 'a missing device must come with a hint');
...
test('switching the adapter while connected drops the old connection instead of mixing transports', async () => {
  await withServer(async (base, server) => {
    ...
    assert.equal(server.backend.state().connected, false, 'the engine must not keep talking over the replaced transport');
...
test('hardware mode runs the full diagnostic path over an injected bus', async () => {
  ...
  try {
    ...
    assert.ok(state.ecus.length >= 2, `expected the virtual ECUs over the injected bus, got ${state.ecus.length}`);
    assert.equal(state.vin, '1HGCM82633A004352', 'the VIN is read over the transport, not from a constant');

    vehicle.setDtc('engine', 'P0420', 0x2f);
    const dtcs = await backend.scanDtcs();
    assert.ok(dtcs.some((dtc) => dtc.code === 'P0420'), 'DTCs are read from the ECU over the injected bus');

    await backend.startLive(undefined);
    await waitFor(() => backend.state().statistics.length > 0, 5000);
    backend.stopLive();
    assert.ok(backend.state().statistics.length > 0, 'live values must be decoded from the injected transport');
  } finally {
    await backend.stop();
    await vehicle.stop();
  }
...
test('a saved session can be replayed without a vehicle', async () => {
  ...
  try {
    ...
    assert.equal(state.connected, true);
    assert.ok(state.ecus.length >= 2, `replay must expose the recorded ECUs, got ${state.ecus.length}`);
