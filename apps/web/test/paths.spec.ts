...
describe("isInsideDirectory — segment comparison", () => {
  ...
  test("a sibling whose name starts with the root name is NOT inside", () => {
    ...
    assert.equal(
      isInsideDirectory(ROOT, `${ROOT}.env`),
      false,
      "the same holds for a file next to the root",
    );
  ...
  test("a hop up and back down is inside, and a hop up past the root is not", () => {
    assert.equal(
      isInsideDirectory(ROOT, join(ROOT, "lib", "..", "app.js")),
      true,
      "resolve folds it back into the root",
    );
    assert.equal(isInsideDirectory(ROOT, join(ROOT, "..", "app.js")), false);
    assert.equal(
      isInsideDirectory(ROOT, join(ROOT, "a", "b", "..", "..", "..", "keys.json")),
      false,
    );
  });
  ...
  test("a trailing separator on the root changes nothing", () => {
    ...
    assert.equal(
      isInsideDirectory(`${ROOT}/`, `${SIBLING}${sep}keys.json`),
      false,
      "and neither does one on the candidate",
    );
    assert.equal(
      isInsideDirectory(`${ROOT}/`, ROOT),
      true,
      "the root is still inside a root written with a slash",
    );
  });
  ...
  test("a relative root is judged from the working directory", () => {
    ...
    assert.equal(
      isInsideDirectory(".", "../outside/file"),
      false,
      'a relative root does not mean "everything"',
    );
  });
  ...
  test("an empty root is the working directory, not a wildcard", () => {
    ...
    assert.equal(
      isInsideDirectory(ROOT, ""),
      false,
      "an empty candidate resolves to the working directory, which is outside a deploy root",
    );
  });
  ...
  test("property: any path built below the root is inside it", () => {
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom("a", "b", "index.html", "sub.dir", "x-y", "ünïcøde", " sp ace "), {
          minLength: 1,
          maxLength: 5,
        }),
        (segments) => {
...
describe("resolveContained — the request path a route hands over", () => {
  test("a plain relative request is joined onto the root", () => {
    ...
    assert.equal(
      resolveContained(ROOT, "lib/chart/core.js"),
      join(ROOT, "lib", "chart", "core.js"),
    );
    assert.equal(
      resolveContained(ROOT, "./app.js"),
      join(ROOT, "app.js"),
      "a leading ./ is decoration",
    );
    assert.equal(
      resolveContained(ROOT, "sub/"),
      resolve(join(ROOT, "sub")),
  ...
  test("an absolute or root-relative request is refused before any resolution", () => {
    assert.equal(
      ...
      null,
      "resolve() would have honoured the last absolute argument and left the root",
    );
    assert.equal(resolveContained(ROOT, "/app.js"), null);
    assert.equal(resolveContained(ROOT, resolve("/etc", "passwd")), null);
    assert.equal(
      resolveContained(ROOT, join(ROOT, "app.js")),
      null,
      "only request-relative paths are accepted here",
    );
  });
  ...
  test("a NUL byte is refused, because the underlying syscalls truncate at it", () => {
    assert.equal(
      resolveContained(ROOT, "app.js\0../../keys.json"),
      null,
      "without the check this reads app.js while looking safe",
    );
  ...
  test("dot and dot-dot segments inside the root stay usable", () => {
    ...
    assert.equal(
      resolveContained(ROOT, ".hidden/rc"),
      join(ROOT, ".hidden", "rc"),
      "dotfiles are contained like any other name",
    );
  });
  ...
  test("property: safe segment lists never escape, and one hop too many is always refused", () => {
    ...
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom(...SAFE), { minLength: 1, maxLength: 6 }),
        (segments) => {
  ...
  test("a name that only looks like the root is refused from the outside in", () => {
    ...
    assert.equal(
      resolveContained(ROOT, `..${sep}${ROOT.split(sep).pop()}-deeper${sep}secret`),
      null,
    );
    expect(resolveContained(SIBLING, "keys.json")).toEqual(join(SIBLING, "keys.json"));
    assert.equal(
      isInsideDirectory(SIBLING, ROOT),
      false,
