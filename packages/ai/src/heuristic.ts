...
import type { AnalysisFinding, AnalysisInput, AnalysisProvider, AnalysisResult, AnalysisSignalSummary } from './types.js';
  ...
  async analyze(input: AnalysisInput): Promise<AnalysisResult> {
    ...
    for (const dtc of input.dtcs) {
      ...
      if (dtc.severity === 'critical' || dtc.severity === 'major') {
        recommendations.push(`${dtc.code} (${dtc.ecu}): ${dtc.description ?? 'diagnose before further use'}`);
      }
    }
    ...
    for (const signal of input.signals) {
      ...
      const spread = Math.abs(signal.average) > 0 ? signal.delta / Math.abs(signal.average) : signal.delta;
    ...
    if (recommendations.length === 0) {
      recommendations.push('Repeat the recording under load to confirm the deviations are reproducible.');
    }
