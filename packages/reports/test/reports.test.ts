...
import { createSession, type VehicleSessionData } from '@vdp/core';
import type { AdapterInfo, TransportInfo } from '@vdp/transport-can';
import { PdfDocument, buildReport, renderHtml, renderPdf, sanitize } from '../src/index.js';
...
function sampleSession(): VehicleSessionData {
  const data = createSession({ adapter: ADAPTER, transport: TRANSPORT, id: 'session_report', title: 'Report test' });
  data.vehicle = {
    vin: '1HGCM82633A004352',
    brand: 'Honda',
    model: 'Accord',
    modelYear: 2003,
    vinAnalysis: { vin: '1HGCM82633A004352', wellFormed: true, checkDigit: 'valid', checkDigitChar: '3', expectedCheckDigitChar: '3', wmi: '1HG', modelYearChar: '3', plantChar: 'A', serial: '004352', notes: [] },
  };
  ...
  data.actions.push({ id: 'a1', timestamp: '2026-09-10T12:00:00.000Z', kind: 'read', ecuId: 'ecu_engine', description: 'Read DTCs', result: 'success' });
  data.notes.push({ id: 'n1', timestamp: '2026-09-10T12:05:00.000Z', text: 'Customer reports rough idle under load.' });
  return data;
}
...
test('report covers every section required by AGENTS 21', () => {
  const document = buildReport({ session: sampleSession(), dtcs: [{ code: 'P0420', severity: 'major', ecu: 'Engine', description: 'Catalyst efficiency below threshold', hint: 'Check lambda sensors first.' }] });
  const headings = document.sections.map((section) => section.heading);
  for (const required of ['Vehicle', 'ECU overview', 'DTC summary', 'Measurements', 'Anomalies', 'Diagnostic actions', 'Notes', 'Recommendations']) {
...
test('recommendations fall back to DTC hints when none are supplied', () => {
  ...
  const recommendations = document.sections.find((section) => section.heading === 'Recommendations');
...
test('HTML rendering escapes untrusted text', () => {
  ...
  session.notes.push({ id: 'n2', timestamp: '2026-09-10T12:06:00.000Z', text: '<script>alert("xss")</script>' });
...
test('PDF rendering produces a non-trivial document with tables', () => {
  const document = buildReport({
    session: sampleSession(),
    dtcs: [{ code: 'P0420', severity: 'major', ecu: 'Engine', description: 'Catalyst efficiency below threshold' }],
    statistics: [
      { signal: 'engine.rpm', name: 'Engine speed', unit: 'rpm', samples: 30, min: 780, max: 4200, average: 2100, delta: 3420, first: 800, last: 900, outOfRangeCount: 0 },
    ],
...
test('long reports paginate instead of overflowing the page', () => {
  const session = sampleSession();
  for (let i = 0; i < 80; i++) {
    session.notes.push({ id: `n${i}`, timestamp: '2026-09-10T12:06:00.000Z', text: `Observation ${i}: the vehicle behaves differently when cold, especially at low RPM and high load.` });
  }
