...
describe('event bus port', () => {
  test('in-memory bus delivers to subscribers and unsubscribes', () => {
    ...
    const seen: string[] = [];
    const unsubscribe = bus.subscribe('dtcs-read', (payload) => seen.push(String(payload.dtcCount)));
...
describe('session store port', () => {
  ...
  test('the default summarizer tolerates sparse sessions and keeps rich ones', async () => {
    type Loose = { id: string; startedAt?: string; endedAt?: string; title?: string; schemaVersion?: number };
    const store = new InMemorySessionStore<Loose>();
    await store.save({ id: 'session_sparse' });
    await store.save({ id: 'session_full', startedAt: '2026-01-01T00:00:00.000Z', endedAt: '2026-01-01T01:00:00.000Z', title: 'Test', schemaVersion: 1 });
    const list = await store.list();
  ...
  test('a custom summarizer wins over the default', async () => {
    const store = new InMemorySessionStore<{ id: string }>((session) => ({ id: session.id, startedAt: 'custom', title: 'derived' }));
...
describe('definition provider port', () => {
  ...
  test('static provider looks up ecus, dids and signals', () => {
    const provider = new StaticDefinitionProvider({
      ...
      ecus: [
        { id: 'engine', name: 'Engine', oem: 'generic', protocol: 'uds', address: { txId: 0x7e0, rxId: 0x7e8 } },
      ],
