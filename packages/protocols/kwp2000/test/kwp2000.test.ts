...
import { KWP_LOCAL_ID, KWP_SID, Kwp2000Client, kwpServiceName } from '../src/index.js';

/** Scripted link: answers with canned payloads or negative responses. */
function createLink(script: (payload: Uint8Array) => Uint8Array): { link: UdsLink; requests: Uint8Array[] } {
...
test('service names are resolvable for logging', () => {
  assert.equal(kwpServiceName(KWP_SID.READ_DATA_BY_LOCAL_IDENTIFIER), 'READ_DATA_BY_LOCAL_IDENTIFIER');
  assert.equal(kwpServiceName(0x99), 'KWP_SERVICE_0x99');
});

test('identification values are read by local identifier', async () => {
  const { link, requests } = createLink(() => fromHex('61 90 31 48 47 43 4D 38 32 36 33 33 41 30 30 34 33 35 32'));
...
test('session start and fault clearing use the KWP2000 services', async () => {
  const { link, requests } = createLink((payload) => new Uint8Array([(payload[0] ?? 0) + 0x40, payload[1] ?? 0]));
  const client = new Kwp2000Client(link);
