...
function createLink(script: (payload: Uint8Array) => Uint8Array, failAfter = Number.POSITIVE_INFINITY): ScriptedLink {
...
describe('request engine', () => {
  ...
  test('a custom timeout overrides the P3 default', async () => {
    ...
    const client = new Kwp2000Client(link, { logger });
    await assert.rejects(client.request(KWP_SID.TESTER_PRESENT, [0x00], 1234), UdsNegativeResponseError);
  });

  test('stats separate requests, responses and negative responses', async () => {
    const link = createLink((payload) => new Uint8Array([(payload[0] ?? 0) + 0x40, payload[1] ?? 0]));
    const client = new Kwp2000Client(link, { logger });
  ...
  test('the client name falls back to the generic label', () => {
    assert.equal(new Kwp2000Client(createLink(() => new Uint8Array(0)), { logger }).name, 'kwp2000-ecu');
  });
});
...
describe('identification reads', () => {
  ...
  test('success returns the payload without the identifier echo', async () => {
    ...
    const client = new Kwp2000Client(link, { logger });
    assert.deepEqual(Array.from((await client.readLocalIdentifier(KWP_LOCAL_ID.DIAGNOSTIC_LEVEL)) ?? []), [0x05]);
  });

  test('session start echoes the accepted session type; short responses fall back', async () => {
    const echo = createLink(() => fromHex('50 89'));
    assert.equal(await new Kwp2000Client(echo, { logger }).startDiagnosticSession(), 0x89, 'default session type 0x89');
    const short = createLink(() => fromHex('50'));
    assert.equal(await new Kwp2000Client(short, { logger }).startDiagnosticSession(0x85), 0x85, 'missing echo falls back');
  });
});
