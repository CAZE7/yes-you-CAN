...
import { DefinitionError, ProtocolError, UdsNegativeResponseError, UdsTimeoutError, createLogger, toHex, type Logger } from '@vdp/shared';
import {
  DID,
  DTC_GROUP_ALL,
  DTC_REPORT,
...
import { decodeDtc, decodeDtcStatus, dtcSeverity, encodeDtcToBytes, type DtcSeverity, type DtcStatusBits } from './dtc.js';
import { DEFAULT_UDS_TIMING, TIMING_MARGIN_MS, parseSessionTiming, type UdsTiming } from './timing.js';
...
export class UdsClient {
  readonly stats: UdsStats = { requests: 0, responses: 0, negativeResponses: 0, pendingResponses: 0, timeouts: 0 };
  ...
  setSeedKeyAlgorithm(algorithm: SeedKeyAlgorithm): void {
    this.log.info('seed&key algorithm registered', { ecu: this.name, algorithm: algorithm.id, provenance: algorithm.provenance });
    this.seedKey = algorithm;
  }

  // ---------------------------------------------------------------- services

  async diagnosticSessionControl(sessionType: number, options: RequestOptions = {}): Promise<{ sessionType: number; p2Ms: number; p2StarMs: number }> {
    const response = await this.request(SID.DIAGNOSTIC_SESSION_CONTROL, [sessionType], options);
    if (response.length === 0) return { sessionType, p2Ms: this.timingValue.p2Ms, p2StarMs: this.timingValue.p2StarMs };
    ...
    this.log.info('session changed', { ecu: this.name, session: this.activeSessionName, ...this.timing });
    return { sessionType: this.activeSession, p2Ms: this.timing.p2Ms, p2StarMs: this.timing.p2StarMs };
  }

  async ecuReset(resetType: number = RESET_TYPE.SOFT_RESET, options: RequestOptions = {}): Promise<Uint8Array> {
    return this.request(SID.ECU_RESET, [resetType], options);
  }

  async testerPresent(suppressPositiveResponse = true, options: RequestOptions = {}): Promise<void> {
    const subFunction = suppressPositiveResponse ? SUPPRESS_POSITIVE_RESPONSE : 0x00;
    await this.request(SID.TESTER_PRESENT, [subFunction], { ...options, suppressPositiveResponse: suppressPositiveResponse });
  }
  ...
  async clearDiagnosticInformation(groupOfDtc: number = DTC_GROUP_ALL): Promise<void> {
    const group = groupOfDtc === DTC_GROUP_ALL ? [0xff, 0xff, 0xff] : [(groupOfDtc >>> 16) & 0xff, (groupOfDtc >>> 8) & 0xff, groupOfDtc & 0xff];
    await this.request(SID.CLEAR_DIAGNOSTIC_INFORMATION, group);
  }

  async readDtcByStatusMask(statusMask: number): Promise<DtcRecord[]> {
    const response = await this.request(SID.READ_DTC_INFORMATION, [DTC_REPORT.REPORT_DTC_BY_STATUS_MASK, statusMask & 0xff]);
    return parseDtcList(response, 3);
  }

  async readSupportedDtc(): Promise<DtcRecord[]> {
    const response = await this.request(SID.READ_DTC_INFORMATION, [DTC_REPORT.REPORT_SUPPORTED_DTC]);
    return parseDtcList(response, 3);
  }

  /** Freeze frame / snapshot data (AGENTS 20 "Freeze Frame / Environment Data"). */
  async readDtcSnapshotRecord(dtc: string, recordNumber = 0xff): Promise<{ recordNumber: number; data: Uint8Array } | null> {
    const bytes = encodeDtcToBytes(dtc);
    const response = await this.request(SID.READ_DTC_INFORMATION, [DTC_REPORT.REPORT_DTC_SNAPSHOT_RECORD_BY_DTC_NUMBER, bytes[0] ?? 0, bytes[1] ?? 0, bytes[2] ?? 0, recordNumber & 0xff]);
    if (response.length < 7) return null;
    return { recordNumber: response[6] ?? recordNumber, data: response.subarray(7).slice() };
  }

  async readDtcExtendedDataRecord(dtc: string, recordNumber = 0x01): Promise<{ recordNumber: number; data: Uint8Array } | null> {
    const bytes = encodeDtcToBytes(dtc);
    const response = await this.request(SID.READ_DTC_INFORMATION, [DTC_REPORT.REPORT_DTC_EXTENDED_DATA_RECORD_BY_DTC_NUMBER, bytes[0] ?? 0, bytes[1] ?? 0, bytes[2] ?? 0, recordNumber & 0xff]);
    if (response.length < 7) return null;
    return { recordNumber: response[6] ?? recordNumber, data: response.subarray(7).slice() };
  }
   ...
   */
  async readDataByIdentifier(dids: readonly number[], lengths?: ReadonlyMap<number, number>): Promise<Map<number, Uint8Array>> {
    ...
    for (const did of dids) {
      const response = await this.request(SID.READ_DATA_BY_IDENTIFIER, [(did >> 8) & 0xff, did & 0xff]);
  ...
  async routineControl(controlType: number, routineId: number, data: Uint8Array = new Uint8Array()): Promise<Uint8Array> {
    const response = await this.request(SID.ROUTINE_CONTROL, [controlType, (routineId >> 8) & 0xff, routineId & 0xff, ...data]);
  ...
  async securityAccessRequestSeed(securityLevel: number, record?: Uint8Array): Promise<Uint8Array> {
    const response = await this.request(SID.SECURITY_ACCESS, [securityLevel & 0xff, ...(record ?? [])]);
   ...
   */
  async unlockSecurityAccess(securityLevel: number, record?: Uint8Array): Promise<{ algorithm: string; provenance: string }> {
    const seed = await this.securityAccessRequestSeed(securityLevel, record);
    const key = await this.seedKey.computeKey({ securityLevel, seed, ...(record ? { record } : {}), ecu: this.name });
  ...
  async request(serviceId: number, data: readonly number[] = [], options: RequestOptions = {}): Promise<Uint8Array> {
    return this.transmit(new Uint8Array([serviceId, ...data]), options, serviceId);
  }

  private async transmit(payload: Uint8Array, options: RequestOptions, serviceId = payload[0] ?? 0): Promise<Uint8Array> {
    this.stats.requests++;
    this.log.raw('uds tx', { ecu: this.name, sid: `0x${serviceId.toString(16)}`, payload: toHex(payload) });
  ...
  private async evaluateResponse(serviceId: number, firstResponse: Uint8Array): Promise<Uint8Array> {
    ...
    for (let pending = 0; ; pending++) {
      ...
      if ((response[0] ?? 0) === NEGATIVE_RESPONSE_SID) {
        ...
        if (nrc === NRC.REQUEST_CORRECTLY_RECEIVED_RESPONSE_PENDING) {
          if (pending >= this.maxPendingResponses) {
            throw new UdsTimeoutError(`ECU stayed in ResponsePending beyond ${this.maxPendingResponses} iterations`, { ecu: this.name });
          }
          this.stats.pendingResponses++;
          this.log.debug('ResponsePending (NRC 0x78) — waiting P2*', { ecu: this.name, p2StarMs: this.timingValue.p2StarMs });
          const next = await this.link.receive(this.timingValue.p2StarMs + TIMING_MARGIN_MS);
          if (!next) {
            this.stats.timeouts++;
            throw new UdsTimeoutError(`no final response after NRC 0x78 within P2* (${this.timingValue.p2StarMs} ms)`, { ecu: this.name });
          }
          response = next;
          continue;
        }
      ...
      throw new ProtocolError(`unexpected UDS response for SID 0x${serviceId.toString(16)}: ${toHex(response)}`, { ecu: this.name });
    }
  }
}
 ...
 */
export function parseMultiDidResponse(response: Uint8Array, lengths: ReadonlyMap<number, number>): Map<number, Uint8Array> {
  const result = new Map<number, Uint8Array>();
  let offset = 1;
  while (offset + 2 <= response.length) {
    ...
    if (length === undefined) {
      throw new DefinitionError(`no definition length for DID 0x${did.toString(16).toUpperCase()} — cannot split a multi-DID response`, {
        did,
      });
    }
    offset += 2;
    if (offset + length > response.length) {
      throw new DefinitionError(`DID 0x${did.toString(16).toUpperCase()} declares ${length} bytes but only ${response.length - offset} remain`, {
        did,
        declared: length,
      });
    }
