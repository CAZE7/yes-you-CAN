...
const validCodeArb = fc
  .tuple(
    fc.constantFrom(...DTC_LETTERS),
    fc.nat({ max: 3 }),
    fc.nat({ max: 15 }),
    fc.nat({ max: 15 }),
    fc.nat({ max: 15 }),
  )
  .map(([letter, d1, d2, d3, d4]) =>
    `${letter}${d1}${d2.toString(16)}${d3.toString(16)}${d4.toString(16)}`.toUpperCase(),
  );
...
describe("DTC status byte round trips (ISO 14229-1 §8.3)", () => {
  ...
  test("property: each generated bit pattern round trips through the named fields", () => {
    fc.assert(
      fc.property(
        fc.record(
          Object.fromEntries(bitNames.map((n) => [n, fc.boolean()])),
        ) as unknown as fc.Arbitrary<Record<(typeof bitNames)[number], boolean>>,
        (bits) => {
          expect(encodeDtcStatus(bits)).toBe(
            encodeDtcStatus(decodeDtcStatus(encodeDtcStatus(bits))),
          );
