...
describe("session timing (ISO 14229-2)", () => {
  test("P2 and P2* are parsed from the session control response", () => {
    assert.deepEqual(parseSessionTiming(new Uint8Array([0x50, 0x03, 0x00, 0x32, 0x01, 0xf4])), {
      p2Ms: 50,
      p2StarMs: 5000,
    });
  });
  ...
  test('short responses and the 0xFFFF "not reported" encodings yield null', () => {
    ...
    assert.equal(
      parseSessionTiming(new Uint8Array([0x50])),
      null,
      "anything shorter than the 6-byte frame is rejected outright",
    );
    assert.equal(
      parseSessionTiming(new Uint8Array([0x50, 0x03, 0xff, 0xff, 0x01, 0xf4])),
      null,
      "P2 = not reported",
    );
    assert.equal(
      parseSessionTiming(new Uint8Array([0x50, 0x03, 0x00, 0x32, 0xff, 0xff])),
      null,
      "P2* = not reported",
    );
  });
  ...
  test("property: P2*/10 ms units convert losslessly", () => {
    fc.assert(
      fc.property(fc.nat({ max: 0xfffe }), fc.nat({ max: 0xfffe }), (p2, p2StarUnits) => {
        const parsed = parseSessionTiming(
          new Uint8Array([
            0x50,
            0x00,
            (p2 >> 8) & 0xff,
...
describe("NRC handling", () => {
  ...
  test("exactly the busy NRCs are transient", () => {
    ...
    assert.equal(
      isTransientNrc(NRC.REQUEST_CORRECTLY_RECEIVED_RESPONSE_PENDING),
      false,
      "pending is a wait, not a retry",
    );
  });
});
...
describe("service constants", () => {
  ...
  test("isPositiveResponse matches only the offset SID", () => {
    ...
    assert.equal(
      isPositiveResponse(0x10, new Uint8Array([])),
      false,
      "empty response is never positive",
    );
  });
