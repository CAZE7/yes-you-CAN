...
function scriptedLink(
  responses: Array<Uint8Array | "timeout">,
  receiveQueue: Array<Uint8Array | null> = [],
): UdsLink & { requests: Uint8Array[] } {
...
const positive = (sid: number, ...rest: number[]): Uint8Array =>
  new Uint8Array([sid + 0x40, ...rest]);
...
describe("response evaluation loop", () => {
  test("ResponsePending beyond maxPendingResponses aborts with UdsTimeoutError", async () => {
    ...
    const client = new UdsClient(link, {
      ...
      timing: { p2Ms: 1, p2StarMs: 1 },
    });
    await expect(client.readDid(DID.VEHICLE_IDENTIFIER_NUMBER)).rejects.toThrow(
      /beyond 2 iterations/,
    );
...
describe("session handling details", () => {
  test("a suppressed positive response keeps the configured timing", async () => {
    ...
    const client = new UdsClient(link, {
      logger,
      sleep: NO_SLEEP,
      timing: { p2Ms: 33, p2StarMs: 660 },
    });
    const result = await client.diagnosticSessionControl(SESSION.EXTENDED, {
...
describe("degenerate parse paths", () => {
  ...
  test("readVin returns null when the ECU returns no usable data", async () => {
    ...
    assert.equal(
      await new UdsClient(noEcho, { logger, sleep: NO_SLEEP }).readVin(),
      null,
      "DID echo mismatch",
    );
    const emptyValue = scriptedLink([positive(0x22, 0xf1, 0x90)]);
    assert.equal(
      await new UdsClient(emptyValue, { logger, sleep: NO_SLEEP }).readVin(),
      null,
      "empty VIN field",
    );
  });
    ...
    assert.deepEqual(
      Array.from(
        parseSingleDidResponse(
          new Uint8Array([0x62, 0xf1, 0x90, 0x57]) as Uint8Array,
          0xf190,
        ) as Uint8Array,
      ),
      [0x57],
  ...
  test("parseMultiDidResponse rejects a truncated tail", () => {
    ...
    assert.throws(
      () => parseMultiDidResponse(response, new Map([[0xf190, 4]])),
      /declares 4 bytes/,
    );
  });
