...
function parseWithId(
  ...
  const dlc = Number.parseInt(dlcChar, 16);
  ...
  for (let i = 0; i < dlc; i++) payload[i] = Number.parseInt(data.slice(i * 2, i * 2 + 2), 16);
  return {
    timestamp: suffix.length === 3 ? now - Number.parseInt(suffix, 16) : now,
...
export function formatSlcanFrame(frame: CanFrame): string {
  if (frame.payload.length > 8) {
    throw new TransportError(`slcan carries at most 8 data bytes, got ${frame.payload.length}`, {
      id: frame.id,
      requested: frame.payload.length,
    });
  }
