...
export function createCanableFactory(stream: ByteStream, defaults: Partial<CanableOptions> = {}): CanAdapterFactory {
  return {
    id: 'canable',
    displayName: 'CANable / CANtact (slcan)',
    create: (options) => new CanableAdapter({ stream, ...defaults, ...(options as Partial<CanableOptions>) }),
