...
export function createSocketCanFactory(binding: SocketCanBinding, defaults: Partial<SocketCanOptions> = {}): CanAdapterFactory {
  return {
    id: 'socketcan',
    displayName: 'SocketCAN (Linux)',
    create: (options) => new SocketCanAdapter({ binding, ...defaults, ...(options as Partial<SocketCanOptions>) }),
