...
import { AdapterCatalog, missingRequiredSettings, type AdapterConfig, type AdapterEntry } from './catalog.js';
...
const ADAPTER_FLAGS = ['adapter', 'device', 'channel', 'bitrate', 'baud', 'trace', 'listen-only', 'configure-port'] as const;
 ...
 */
export function parseAdapterArgv(argv: readonly string[], defaultId = DEFAULT_ADAPTER_ID): ParsedAdapterOptions {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    const value = inlineValue ?? (nextValue !== undefined && !nextValue.startsWith('--') ? nextValue : undefined);
    ...
    if (value === undefined || value.length === 0) {
      errors.push(`--${name} requires a value, e.g. --${name}=<${name === 'adapter' ? 'id' : 'value'}>`);
      continue;
    }
    if (name === 'adapter') id = value;
      ...
      if (!Number.isFinite(baud) || baud <= 0) errors.push(`--baud must be a positive integer, got "${value}"`);
 ...
 */
export function validateSelection(catalog: AdapterCatalog, selection: AdapterSelection): SelectionValidation {
  const errors: string[] = [];
  ...
  if (entry) {
    ...
    if (resolved.bitrate && entry.supportedBitrates && !entry.supportedBitrates.includes(resolved.bitrate)) {
      errors.push(`unsupported bitrate "${resolved.bitrate}" for ${entry.id} (supported: ${entry.supportedBitrates.join(', ')})`);
    }
 ...
 */
export function selectionFromPayload(payload: unknown, defaultId = DEFAULT_ADAPTER_ID): AdapterSelection {
  ...
  else if (typeof baud === 'string' && /^\d+$/.test(baud)) config.baudRate = Number.parseInt(baud, 10);
...
export function formatAdapterHelp(catalog: AdapterCatalog): string {
  ...
  lines.push('');
  lines.push('Common settings: --device=<path> --channel=<iface> --bitrate=<500k|250k|...> --baud=<bits/s>');
