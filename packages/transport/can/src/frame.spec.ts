  ...
  test('property: illegal FD lengths pad up to the next legal frame length, never down', () => {
    ...
    fc.assert(
      fc.property(fc.nat({ max: CANFD_MAX_PAYLOAD }), (length) => {
        ...
        assert.equal(lengthToDlc(padded, true), lengthToDlc(length, true), 'the chosen DLC must be minimal');
      }),
    );
  });
...
describe('frameMatchesFilters', () => {
  ...
  test('extended flag gates a filter when declared', () => {
    assert.equal(frameMatchesFilters(frame(0x123, true), [{ id: 0x123, mask: 0x7ff, extended: true }]), true);
    assert.equal(frameMatchesFilters(frame(0x123, false), [{ id: 0x123, mask: 0x7ff, extended: true }]), false);
