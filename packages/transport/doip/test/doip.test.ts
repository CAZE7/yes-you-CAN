...
class FakeDoipEndpoint implements DoipSocket {
  ...
  async send(data: Uint8Array): Promise<void> {
    ...
    if (header.payloadType === PAYLOAD_TYPE.DIAGNOSTIC_MESSAGE) {
      ...
      const reply = this.responder?.(decoded.udsPayload) ?? new Uint8Array([(decoded.udsPayload[0] ?? 0) + 0x40, 0x00]);
...
test('DoIP header encodes version, inverse version, type and length', () => {
  const message = encodeMessage(PAYLOAD_TYPE.ROUTING_ACTIVATION_REQUEST, new Uint8Array([0x0e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]));
...
test('vehicle identification responses round trip', () => {
  ...
  const decoded = decodeVehicleIdentificationResponse(encodeVehicleIdentificationResponse(original));
...
test('connect() performs routing activation and stores the entity address', async () => {
  ...
  const transport = new DoipTransport({ socket: endpoint, targetAddress: 0x1000, activationTimeoutMs: 500 });
...
test('UDS runs over DoIP without the client knowing the transport (AGENTS 5)', async () => {
  ...
  endpoint.responder = (uds) => {
    if ((uds[0] ?? 0) === 0x22) return fromHex('62 F1 90 31 48 47 43 4D 38 32 36 33 33 41 30 30 34 33 35 32');
    return new Uint8Array([(uds[0] ?? 0) + 0x40]);
  };
  const transport = new DoipTransport({ socket: endpoint, targetAddress: 0x1000, activationTimeoutMs: 500 });
  await transport.connect();
  const client = new UdsClient(createRequestResponseLink(transport, { defaultTimeoutMs: 500 }), { name: 'doip-ecu' });
...
test('TLS can be enforced (AGENTS 8 security note, AGENTS 26/27)', async () => {
  ...
  const secureTransport = new DoipTransport({ socket: secure, targetAddress: 0x1000, requireTls: true });
...
test('discovery broadcasts the identification request and parses announcements', async () => {
  ...
  const socket = {
    async broadcast(data: Uint8Array) {
      ...
      listener?.(encodeMessage(PAYLOAD_TYPE.VEHICLE_ANNOUNCEMENT_RESPONSE, encodeVehicleIdentificationResponse({
        vin: '1HGCM82633A004352',
        logicalAddress: 0x1000,
        eid: fromHex('00 11 22 33 44 55'),
        gid: fromHex('66 77 88 99 AA BB'),
  ...
  assert.equal(decodeHeader(sent[0] as Uint8Array).payloadType, PAYLOAD_TYPE.VEHICLE_IDENTIFICATION_REQUEST);
