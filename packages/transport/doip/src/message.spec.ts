      ...
      fc.property(
        ...
        (payload, source, target) => {
          const message = encodeMessage(
            ...
            encodeDiagnosticMessage(source, target, payload),
          );
        ...
        (vin, logicalAddress, eid, gid, furtherAction, vinSyncStatus) => {
          const decoded = decodeVehicleIdentificationResponse(
            encodeVehicleIdentificationResponse({
              ...
              ...(vinSyncStatus !== undefined ? { vinSyncStatus } : {}),
            }),
          );
...
describe("routing activation (ISO 13400-2 §9.3)", () => {
  test("the response layout is the one ISO 13400-2 writes on the wire", () => {
    ...
    assert.equal(
      toHex(encodeRoutingActivationResponse(0x0e80, 0x0001, 0x10)),
      "0E 80 00 01 10 00 00 00 00",
    );
    const decoded = decodeRoutingActivationResponse(fromHex("0E 80 00 01 10 11 22 33 44"));
    assert.deepEqual(
      {
        tester: decoded.testerLogicalAddress,
        entity: decoded.entityLogicalAddress,
        code: decoded.code,
      },
  ...
  test("success and known codes carry their names, unknown codes stay reportable", () => {
    assert.equal(
      decodeRoutingActivationResponse(encodeRoutingActivationResponse(0x0e80, 0x0001, 0x10))
        .codeName,
      "success",
    );
    assert.match(
      decodeRoutingActivationResponse(encodeRoutingActivationResponse(0x0e80, 0x0001, 0x2a))
        .codeName,
      /unknown_0x2a/,
    );
  });
  ...
  test("request embeds the activation type", () => {
    const request = encodeRoutingActivationRequest(
      0x0e80,
      ROUTING_ACTIVATION_TYPE.CENTRAL_SECURITY,
    );
...
describe("diagnostic message + ack", () => {
  test("property: payload survives the diagnostic round trip byte for byte", () => {
    fc.assert(
      fc.property(
        fc.uint8Array({ minLength: 0, maxLength: 4095 }),
        addressArb,
        addressArb,
        (udsPayload, source, target) => {
          const decoded = decodeDiagnosticMessage(
            encodeDiagnosticMessage(source, target, udsPayload),
          );
          assert.equal(decoded.sourceAddress, source);
          assert.equal(decoded.targetAddress, target);
  ...
  test("ack decodes source, target and code", () => {
    ...
    assert.deepEqual(decodeDiagnosticAck(ack), {
