...
describe('decoded sample CSV (SessionLogger.toCsv)', () => {
  test('an empty export is the header alone, still newline terminated', () => {
    const csv = SessionLogger.toCsv([]);
    assert.equal(csv, 'timestamp,t_ms,signal,value,raw_value,raw_hex,unit,enum_text,out_of_range\n');
    assert.deepEqual(parseCsv(csv), [['timestamp', 't_ms', 'signal', 'value', 'raw_value', 'raw_hex', 'unit', 'enum_text', 'out_of_range']]);
  });
  ...
  test('every row has nine fields, in the header order, and nothing else', () => {
    ...
    for (const row of rows) assert.equal(row.length, 9, 'a fixed column count is what lets a spreadsheet load the file');
    assert.deepEqual(rows[1], ['2026-09-11T08:00:01.000Z', '1000', 'engine.rpm', '847', '105', '0069', '', '', 'false']);
    assert.deepEqual(rows[2], ['2026-09-11T08:00:01.000Z', '1000', 'engine.limp_mode', 'true', '1', '01', '', '', 'false']);
  });
  ...
  test('unit and enum_text appear only when the sample has them', () => {
    ...
    assert.ok(!csv.includes('undefined') && !csv.includes('null'), 'an absent field is an empty cell, never a JS type name');
  });
  ...
  test('values are written the way they were decoded, without a formatting layer in between', () => {
    ...
    assert.deepEqual(parseCsv(csv).slice(1).map((row) => row[3]), [
      '42',
      '0.1',
      'false',
      'ABCD',
    ...
    assert.deepEqual(parseCsv(csv).slice(1).map((row) => row[4]), ['42', '1', '0', 'ABCD', 'NaN', '1.7976931348623157e+308', '0', '-5']);
  });

  test('out_of_range is a lower-case boolean', () => {
    const csv = SessionLogger.toCsv([sample({ value: 1, outOfRange: true }), sample({ signal: 'x', value: 2, outOfRange: false })]);
    assert.deepEqual(parseCsv(csv).slice(1).map((row) => row[8]), ['true', 'false']);
  });

  test('commas, quotes and newlines survive the round trip', () => {
    const awkward = 'knock, "loud"\r\nsecond line';
    const csv = SessionLogger.toCsv([sample({ signal: 'engine.note', value: awkward, rawValue: awkward, rawHex: '"' })]);
    assert.ok(csv.includes('"knock, ""loud""\r\nsecond line"'), 'the field is quoted and its quotes doubled');
    ...
    assert.equal(parseCsv(csv).length, 2, 'an embedded newline does not invent a row for a reader that respects quoting');
  });
  ...
  test('the marker block follows the data, only when there are markers', () => {
    ...
    const csv = SessionLogger.toCsv(
      [sample()],
      [
        { id: 'marker_1', t: 0, timestamp: '2026-09-11T08:00:00.000Z', label: 'ignition on', kind: 'user' },
        { id: 'marker_2', t: 5, timestamp: '2026-09-11T08:00:05.000Z', label: 'key on, cold', detail: 'engine cold', kind: 'action' },
      ],
    );
    const [header, data, blank, markerTitle, markerHeader, first, second] = csv.trimEnd().split('\n');
    assert.equal(header, 'timestamp,t_ms,signal,value,raw_value,raw_hex,unit,enum_text,out_of_range');
    assert.equal(blank, '', 'a blank line separates the two blocks so a strict reader can cut here');
      ...
      fc.property(
        fc.array(
          fc.record({
            ...
            rawHex: fc
              .array(fc.nat({ max: 255 }), { maxLength: 6 })
              .map((bytes) => bytes.map((byte) => byte.toString(16).padStart(2, '0')).join('').toUpperCase()),
        ...
        (rows) => {
          ...
          for (const [index, row] of parsed.slice(1).entries()) {
            ...
            assert.equal(row?.[2], source.signal, 'the id column can hold any string, including commas and quotes');
...
describe('raw trace and JSON exports', () => {
  test('traceToCsv of no frames is the header alone', () => {
    assert.equal(SessionLogger.traceToCsv([]), 'timestamp,t_ms,can_id,direction,dlc,payload,channel,extended,fd\n');
  });
  ...
  test('one row per frame, with the id in hex and the payload compact', () => {
    ...
    assert.deepEqual(rows[1], rows[2], 'the entry handed to the caller is the entry that was stored');
    assert.deepEqual(rows[1], ['1970-01-01T00:00:00.000Z', '0', '0x7E8', 'tx', '2', 'DEAD', 'can0', 'false', 'false']);
  });
  ...
  test('toJson is the lossless form: hex payload plus the full sample objects', () => {
    ...
    assert.equal(parsed.trace[0]?.payload, '62F1', 'bytes leave as hex, so JSON stays a text format');
    assert.equal(parsed.trace[0]?.payloadHex, '62F1');
    assert.deepEqual(Object.keys(parsed).sort(), ['dtcs', 'exportedAt', 'format', 'formatVersion', 'log', 'markers', 'measurements', 'meta', 'trace']);
    assert.equal(new Date(parsed.exportedAt).toISOString(), parsed.exportedAt, 'exportedAt is the wall clock of the export, not of the session');
...
describe('what a session object guarantees around the exports', () => {
  ...
  test('t is relative to the session clock start, and a backwards clock stays negative', () => {
    let now = 1_000;
    const session = new SessionLogger({ clock: () => now });
    assert.equal((session.recordFrame(frameAt(0x7e0, [1], now, 'tx')) as { t: number }).t, 0, 'the first frame is the zero point');
    now = 1_500;
  ...
  test('a log entry without fields has no fields key at all', () => {
    const session = new SessionLogger({ clock: () => 0 });
    assert.deepEqual(Object.keys(session.log('dtc', 'scanned 3 codes')), ['timestamp', 't', 'scope', 'message']);
    assert.deepEqual(Object.keys(session.log('dtc', 'detail', { count: 3 })), ['timestamp', 't', 'scope', 'message', 'fields']);
  });
  ...
  test('a frame without a timestamp falls back to the session clock', () => {
    ...
    assert.equal(entry.timestamp, new Date(7).toISOString(), '0 means "no timestamp", so the clock answers');
