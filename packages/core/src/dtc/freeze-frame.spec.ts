...
describe("undocumented records", () => {
  ...
  test("an empty record and a definition without layout produce distinct notes", () => {
    ...
    const noLayout = decodeFreezeFrame(PAYLOAD, {
      code: "P0420",
      recordNumber: 1,
      definition: { code: "P0420", description: "x" },
    });
...
describe("documented records", () => {
  test("fields are split in declaration order and signals decoded with scale", () => {
    const frame = decodeFreezeFrame(PAYLOAD, {
      code: "P0301",
      recordNumber: 0x02,
      fields: layout,
      signals: SIGNALS,
    ...
    assert.deepEqual(frame.notes, [
      "field 0xF40C has no signal definitions — its bytes are reported without interpretation",
    ]);
  ...
  test("field length can be derived from the signals instead of a declared length", () => {
    ...
    const frame = decodeFreezeFrame(new Uint8Array([0x00, 0x64, 0xff, 0xff]), {
      code: "P0420",
      recordNumber: 1,
      fields,
      signals: SIGNALS,
  ...
  test("a record shorter than the layout stops cleanly at the missing field", () => {
    const frame = decodeFreezeFrame(new Uint8Array([0x0b, 0xb8]), {
      code: "P0420",
      recordNumber: 1,
      fields: layout,
      signals: SIGNALS,
    ...
    const twoFields = decodeFreezeFrame(new Uint8Array([0x0b, 0xb8, 0x07, 0xd0]), {
      code: "P0420",
      recordNumber: 1,
      fields: layout,
      signals: SIGNALS,
...
describe("broken layouts", () => {
  test("a field with neither length nor signals aborts the split with a note", () => {
    ...
    assert.ok(
      frame.notes.some((note) => note.includes("could not be split with the documented layout")),
    );
    assert.equal(frame.unassignedHex, "0B B8 07 D0 2A");
  });
  ...
  test("unknown signal ids are ignored; a fully unknown field reports its bytes uninterpreted", () => {
    ...
    const frame = decodeFreezeFrame(new Uint8Array([0x00, 0x64]), {
      code: "P0420",
      recordNumber: 1,
      fields,
      signals: SIGNALS,
  ...
  test("a field without signal definitions is reported without interpretation", () => {
    ...
    const frame = decodeFreezeFrame(new Uint8Array([0xaa, 0xbb]), {
      code: "P0420",
      recordNumber: 1,
      fields,
      signals: SIGNALS,
