...
import { DEFAULT_MIN_BATTERY_VOLTAGE, SafetyManager, type VehicleState, type WriteRequestContext } from './safety-manager.js';

const OK_STATE: VehicleState = { stationary: true, batteryVoltage: 12.5, ignitionOn: true, parkingBrake: true };
...
describe('evaluate — precondition catalogue', () => {
  ...
  test('vehicle state: moving, ignition off, and low battery each block independently', () => {
    ...
    const result = manager.evaluate(OK_CONTEXT, { ...OK_STATE, stationary: false, ignitionOn: false, batteryVoltage: 11.4 });
    ...
    const brakeOnly = new SafetyManager().evaluate(OK_CONTEXT, { ...OK_STATE, parkingBrake: false });
  ...
  test('an unknown battery voltage is a warning, not a free pass', () => {
    ...
    assert.equal(result.warnings.includes('battery voltage unknown — precondition not verifiable'), true);
  ...
  test('the configured minimum voltage wins over the default', () => {
    const result = new SafetyManager({ minBatteryVoltage: 13.2 }).evaluate(OK_CONTEXT, { ...OK_STATE, batteryVoltage: 12.5 });
    assert.equal(result.failed.some((f) => f.includes('13.20 V')), true);
  ...
  test('low risk does not demand the parking brake; medium and high do', () => {
    ...
    assert.deepEqual(high.warnings, ['high risk operation — consider a workshop-grade power supply']);
  ...
  test('a partial type match (only one side known) is not treated as a mismatch', () => {
    const result = new SafetyManager().evaluate({ ...OK_CONTEXT, expectedEcuType: 'BCM' }, OK_STATE);
  ...
  test('DoIP network preconditions are checked when provided', () => {
    ...
    const result = manager.evaluate(
      { ...OK_CONTEXT, network: { tlsActive: false, routingActivationOk: false, unauthorizedDevicesInSegment: true } },
      OK_STATE,
    );
    assert.deepEqual(result.failed, ['DoIP transport is not using TLS', 'DoIP routing activation failed', 'unauthorized devices detected in the diagnostic network segment']);
    assert.equal(manager.evaluate({ ...OK_CONTEXT, network: { tlsActive: true, routingActivationOk: true } }, OK_STATE).ok, true);
  });
});
...
describe('permits', () => {
  test('a passing request issues a TTL-bounded permit and writes the audit log', () => {
    ...
    assert.deepEqual(manager.audit.map((entry) => entry.action), ['permit-issued']);
  });

  test('a failing request throws SafetyViolationError with every reason and audits the denial', () => {
    const manager = new SafetyManager();
    assert.throws(
      () => manager.requestPermit({ ...OK_CONTEXT, backupAvailable: false, userConfirmed: false }, OK_STATE),
      (error: unknown) => {
        ...
        assert.deepEqual((error as SafetyViolationError & { failedPreconditions: string[] }).failedPreconditions, [
          'no backup available — rollback would be impossible',
          'user confirmation missing',
        ]);
        return true;
      },
    );
    assert.deepEqual(manager.audit.map((entry) => entry.action), ['permit-denied']);
  });
