...
describe('metadata comparison', () => {
  ...
  test('a different VIN is critical and lands in the summary', () => {
    const result = compareSessions(side(), side({ id: 's2', label: 'nachher', vin: '1HGCM82633A004353' }));
    assert.equal(result.critical.length, 1);
    assert.equal(result.critical[0]?.field, 'vin');
    assert.ok(result.summary.some((line) => line.includes('VIN')), 'the summary must warn loudly');
  });

  test('a definition package change is critical, an adapter change is not', () => {
    const result = compareSessions(
      side(),
      side({ id: 's2', label: 'nachher', definitionPackage: { oem: 'generic', version: '2.0.0' }, adapter: 'SocketCAN' }),
    );
    assert.deepEqual(
...
describe('fault code comparison', () => {
  ...
  test('an unchanged fault memory is reported as such', () => {
    const result = compareSessions(side({ dtcs: [dtc('P0420', 0x08)] }), side({ id: 's2', label: 'nachher', dtcs: [dtc('P0420', 0x08)] }));
...
describe('signal comparison', () => {
  ...
  test('signalIds restricts the comparison and deltas are right - left', () => {
    const result = compareSessions(
      ...
      side({ id: 's2', label: 'nachher', samples: [sample('engine.rpm', 2100), sample('engine.coolant', 90)] }),
