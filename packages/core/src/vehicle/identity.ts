...
export function createIdentityFromVin(vin: string, extra: Partial<VehicleIdentity> = {}): VehicleIdentity {
  const analysis = analyseVin(vin);
  return {
    ...extra,
    vin: analysis.vin,
    vinAnalysis: analysis,
    modelYear: extra.modelYear ?? (analysis.wellFormed ? (guessModelYear(analysis.modelYearChar) ?? undefined) : undefined),
    ecus: extra.ecus ?? [],
...
export function describeVehicle(identity: VehicleIdentity | undefined): string {
  ...
  const parts = [identity.brand, identity.model, identity.modelYear ? String(identity.modelYear) : undefined]
