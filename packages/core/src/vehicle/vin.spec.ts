...
const vinArb = fc
  .array(vinCharArb, { minLength: 17, maxLength: 17 })
...
describe("check digit (position 9, ISO 3779 / 49 CFR 565)", () => {
  test("a VIN whose position 9 carries its own check digit validates", () => {
    ...
    assert.equal(
      analyseVin("1HGCM82633A004352").expectedCheckDigitChar,
      computeVinCheckDigit("1HGCM82633A004352"),
    );
  });
...
describe("computeVinCheckDigit — the arithmetic itself", () => {
  ...
  test("the check digit position itself is weighted with zero", () => {
    ...
    for (const char of VIN_CHARS) {
      ...
      assert.equal(
        computeVinCheckDigit(tampered),
        without,
        `position 9 char "${char}" must not change the computed digit`,
      );
    }
  });
  ...
  test("characters ISO 3779 forbids transliterate to zero instead of failing", () => {
    ...
    assert.equal(
      computeVinCheckDigit("QHGCM82633A004352"),
      computeVinCheckDigit("0HGCM82633A004352"),
    );
    assert.equal(
      computeVinCheckDigit("IIGCM82633A004352"),
      computeVinCheckDigit("00GCM82633A004352"),
    );
    assert.notEqual(
      computeVinCheckDigit("QHGCM82633A004352"),
      computeVinCheckDigit("1HGCM82633A004352"),
      "a real character would have counted as 1",
    );
    assert.match(
      computeVinCheckDigit("IIOOQQ"),
      /^[0-9X]$/,
      "still exactly one check digit character",
    );
  });
  ...
  test("property: fixing position 9 makes the VIN validate, breaking it breaks validation unless the value coincides", () => {
    fc.assert(
      fc.property(vinArb, (generated) => {
        ...
        assert.equal(
          computeVinCheckDigit(correct),
          digit,
          "the computation is stable under its own result",
        );
        ...
        for (const other of ["0", "1", "5", "9", "X"]) {
          ...
          assert.equal(
            analyseVin(broken).checkDigit,
            "invalid-check-digit",
            `position 9 = "${other}" must not validate`,
          );
        }
      }),
      { numRuns: 150 },
...
describe("analyseVin — the report a session stores", () => {
  ...
  test("the Tesla example shows a read error being distinguishable from a different vehicle", () => {
    ...
    assert.equal(
      analysis.modelYearChar,
      "K",
      "model year and plant are still readable from a VIN with a bad check digit",
    );
  ...
  test("every structural field is sliced positionally, even when the input is malformed", () => {
    ...
    assert.equal(analysis.checkDigit, "malformed");
    assert.equal(
      analysis.expectedCheckDigitChar,
      "",
      "nothing is computed for a VIN that is not 17 characters",
    );
  ...
  test("every problem is reported at once, in the documented order", () => {
    ...
    assert.ok(
      analyseVin("").notes.length === 1,
      "an empty VIN only has the length problem to report",
    );
  });
  ...
  test("a correct length with a forbidden character reports the character, not the length", () => {
    ...
    assert.equal(
      ...
      2,
      "the O is both forbidden and outside the alphabet — no length note",
    );
    ...
    assert.equal(
      analysis.checkDigit,
      "malformed",
      "and the check digit is never judged on a malformed VIN",
    );
  });
    ...
    fc.assert(
      fc.property(fc.string({ maxLength: 24 }), (candidate) => {
        ...
        if (analysis.wellFormed) {
          assert.deepEqual(
            analysis.notes,
            analysis.checkDigit === "valid" ? [] : [analysis.notes[0] as string],
          );
...
describe("guessModelYear — picking a cycle", () => {
  ...
  test("a letter code with three cycles picks the newest one the reference year allows", () => {
    ...
    assert.equal(
      guessModelYear("A", 2038),
      2010,
      "2040 is two model years ahead, so the 1980 cycle still wins",
    );
    assert.equal(
      guessModelYear("A", 2039),
      2040,
      "one year ahead is allowed — the next model year is sold early",
    );
    assert.equal(guessModelYear("A", 2040), 2040);
    assert.equal(
      guessModelYear("A", 2041),
      2040,
      "and once it is the current year it simply stays the answer",
    );
  });
  ...
  test("without a reference year the current calendar year is used", () => {
    ...
    for (const code of MODEL_YEAR_CHARS) {
      ...
      assert.ok(
        candidates.includes(guess),
        `${code}: ${guess} must be one of ${candidates.join(", ")}`,
      );
      assert.ok(
        guess <= current + 1,
        `${code}: ${guess} may not be more than one year ahead of ${current}`,
      );
    ...
    fc.assert(
      fc.property(
        ...
        (code, reference) => {
          ...
          const expected =
            candidates.filter((year) => year <= reference + 1).pop() ?? (candidates[0] as number);
