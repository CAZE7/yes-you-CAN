...
import { type EcuReader, LiveDataEngine } from "./live.js";
...
function reader(
  ecuId: string,
  payloads: Map<number, Uint8Array | null>,
  throws: number[] = [],
): EcuReader & { calls: number[] } {
  const calls: number[] = [];
  return {
    ecuId,
    calls,
...
describe("LiveDataEngine", () => {
  test("polls each ECU serially per round and groups signals by DID", async () => {
    ...
    const enginePlan = new Map<string, readonly SignalDefinition[]>([
      [
        "engine",
        [signal(0x0c, "engine.rpm"), signal(0x0c, "engine.rpm2"), signal(0x05, "engine.coolant")],
      ],
    ]);
  ...
  test("missing payloads and reader errors become per-DID errors, not crashes", async () => {
    ...
    const result = await engine.run(
      [flaky],
      new Map([["engine", [signal(0x0c), signal(0x05), signal(0x11)]]]),
    );
  ...
  test("double start is rejected", async () => {
    ...
    const run = engine.run(
      [reader("engine", new Map([[0x0c, new Uint8Array([0, 1])]]))],
      new Map([["engine", [signal(0x0c)]]]),
    );
