...
function decode(definition: SignalDefinition, payload: Uint8Array): { value: number; outOfRange: boolean; rawValue: number | string | boolean } {
  const decoded = lenient.decode(definition, payload);
  assert.ok(decoded, `expected the window ${definition.byteOffset}+${definition.length} of payload "${toHex(payload)}" to decode`);
  return { value: decoded.value as number, outOfRange: decoded.outOfRange, rawValue: decoded.rawValue };
}
...
describe('declared range (min/max)', () => {
  test('the boundaries themselves are inside the range', () => {
    ...
    assert.equal(decode(definition, fromHex('00 0A')).outOfRange, false, 'value === min is in range');
    assert.equal(decode(definition, fromHex('00 14')).outOfRange, false, 'value === max is in range');
  ...
  test('a range with only a lower bound still flags above it, and vice versa', () => {
    ...
    assert.equal(decode(minOnly, fromHex('FF FF')).outOfRange, false, '65535 is above the only bound, that is fine');
    const maxOnly = signal({ max: 100 });
    assert.equal(decode(maxOnly, fromHex('00 65')).outOfRange, true, '101 exceeds max');
    assert.equal(decode(maxOnly, fromHex('FF FF')).value, 65_535, 'the value is still reported — a flag is not a refusal');
  });
  ...
  test('scaling happens first, so the range is about the physical value', () => {
    ...
    assert.deepEqual(decode(definition, fromHex('03 84')), { value: 50, outOfRange: false, rawValue: 900 });
  ...
  test('the offset shifts the window check as well', () => {
    ...
    assert.equal(decode(definition, fromHex('00 28')).outOfRange, false, '0 is the lower boundary, inclusive');
    assert.equal(decode(definition, fromHex('00 27')).outOfRange, true, '39 raw - 40 = -1, below min');
  });
  ...
  test('the range check sees the value the user is shown, i.e. the rounded one', () => {
    ...
    assert.deepEqual(decode(half, fromHex('02')), { value: 1, outOfRange: false, rawValue: 2 }, '1.0 === min is inside');
    assert.deepEqual(decode(half, fromHex('01')), { value: 0.5, outOfRange: true, rawValue: 1 }, '0.5 is below min');
    assert.deepEqual(decode(half, fromHex('15')), { value: 10.5, outOfRange: true, rawValue: 21 }, '10.5 is above max');
  ...
  test('a bit-field range is checked on the extracted bits, scaled', () => {
    ...
    const high = signal({ length: 1, encoding: 'bitmask', bitOffset: 0, bitLength: 4, min: 0, max: 8 });
    ...
    const low = signal({ length: 1, encoding: 'bitmask', bitOffset: 4, bitLength: 4, min: 0, max: 8 });
    assert.equal(decode(low, fromHex('F0')).value, 0, 'and the same byte has nothing in its low nibble');
  ...
  test('text and boolean signals are never out of range, whatever min/max say', () => {
    ...
    assert.ok(decodedText && !('unit' in decodedText), 'no unit was declared, so the key is absent rather than empty');
    ...
    assert.equal(lenient.decode(bool, fromHex('01'))?.outOfRange, false, 'a boolean has no numeric range to leave');
    assert.equal(lenient.decode(bool, fromHex('00'))?.value, false, 'every non-zero byte means true, zero means false');
  });

  test('an enum mapping does not influence the range decision', () => {
    const definition = signal({ min: 0, max: 1, enumMapping: { 0: 'off', 1: 'on', 2: 'error state' } });
  ...
  test('a non-finite value is reported as it is: Infinity leaves the range, NaN cannot', () => {
    ...
    assert.equal(lenient.decode(inf, new Uint8Array(view.buffer))?.outOfRange, true, 'and -Infinity below any finite min');
  });
  ...
  test('an out-of-range value is a WARN with the numbers that produced it', () => {
    ...
    assert.deepEqual(logger.records.filter((record) => record.message.includes('outside declared range')), [], 'an in-range value is silent');
...
describe('general decoding', () => {
  test('endianness reverses the window, not the digits of the value', () => {
    assert.equal(decode(signal({ length: 2 }), fromHex('12 34')).value, 0x1234, 'big endian is the default');
    assert.equal(decode(signal({ length: 2, endianness: 'little' }), fromHex('12 34')).value, 0x3412);
    assert.equal(decode(signal({ encoding: 'int16', length: 2, endianness: 'little' }), fromHex('d8 ff')).value, -40, 'FFD8 little endian is -40');
    ...
    assert.equal(decode(signal({ encoding: 'float32', length: 4, endianness: 'little' }), fromHex('00 00 C0 3F')).rawValue, 1.5);
    assert.equal(decode(signal({ encoding: 'float32', length: 4, scale: 0.1 }), fromHex('3F C0 00 00')).value, 0.2, '0.15 at the resolution of a 0.1 step');
  });
  ...
  test('scale and offset are applied as written, and only to numeric values', () => {
    ...
    assert.equal(decode(signal({ scale: 0.1, offsetValue: -40 }), fromHex('00 64')).value, -30, 'offset after scaling');
    assert.equal(decode(signal({ offsetValue: 5 }), fromHex('00 0A')).value, 15, 'a scale of 1 is the identity');
    assert.equal(decode(signal({ scale: -0.1, offsetValue: 100 }), fromHex('00 64')).value, 90, 'an inverted sensor still gets one decimal, not NaN');
  });
  ...
  test('the reported value is rounded to the decimals the scale can carry', () => {
    ...
    assert.equal(decode(signal({ encoding: 'uint8', length: 1, scale: 0.01 }), fromHex('FF')).value, 2.55);
    assert.equal(decode(signal({ encoding: 'uint8', length: 1, scale: 10 }), fromHex('FF')).value, 2550, 'a coarse scale has no decimals to keep');
    ...
    assert.equal(decode(unscaled, fromHex('3F C0 00 00')).value, 2, '1.5 rounded to the scale resolution');
    assert.equal(decode(unscaled, fromHex('3F C0 00 00')).rawValue, 1.5, 'and the raw value is untouched');
  ...
  test('a bit mask is a pattern, not a measurement: no scaling, and no bit window means the whole window', () => {
    ...
    assert.equal(decode(signal({ encoding: 'bitmask', length: 1, scale: 0.1, offsetValue: 100 }), fromHex('0A')).value, 10, 'scale and offset are ignored');
    assert.equal(decode(signal({ encoding: 'bitmask', length: 2 }), fromHex('01 00')).value, 256, 'two bytes of pattern are read big endian');
  });
  ...
  test('text stops at the first NUL and loses its padding', () => {
    ...
    assert.equal(lenient.decode(text, fromHex('20 20 41 42 20 20 00 00'))?.value, 'AB', 'a padded field is trimmed');
    assert.equal(lenient.decode(text, fromHex('00 00 00 00 00 00 00 00'))?.value, '', 'an empty field is an empty string, not a failure');
    const high = signal({ encoding: 'ascii', length: 4 });
    assert.equal(lenient.decode(high, fromHex('FF FE 41 00'))?.value, '\u00ff\u00feA', 'a byte above ASCII is passed through as its code point');
    const raw = lenient.decode(high, fromHex('41 00 42 00'));
    assert.equal(raw?.rawHex, '41 00 42 00', 'the raw window is kept whole even though the text is shorter');
  ...
  test('an enum is looked up by the raw value, and an unmapped code stays a number', () => {
    const scaled = signal({ encoding: 'uint8', length: 1, scale: 2, enumMapping: { 1: 'one', 3: 'three' } });
    ...
    const mapped = lenient.decode(signal({ encoding: 'uint8', length: 1, enumMapping: { 1: 'one', 3: 'three' } }), fromHex('03'));
    assert.deepEqual(
    ...
    const unmapped = lenient.decode(signal({ encoding: 'uint8', length: 1, enumMapping: { 1: 'one' } }), fromHex('03'));
  ...
  test('identity fields come from the definition untouched, so a chart can key on them', () => {
    const definition = signal({ id: 'abs.wheel_speed_fl', name: 'Wheel speed, front left', ecu: 'abs', did: 0xf1a2, unit: 'km/h', length: 2 });
...
describe('payload bounds', () => {
  ...
  test('property: a window that does not fit never decodes, whatever the offset', () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 6 }), fc.integer({ min: 1, max: 6 }), (offset, length) => {
        ...
        expect(() => strict.decode(definition, payload)).toThrow(/cannot decode engine\.edge_case/);
      }),
      { numRuns: 60 },
    );
  });
  ...
  test('an empty payload is reported, never decoded as zeros', () => {
    for (const encoding of ['uint8', 'uint16', 'int32', 'float32', 'ascii', 'bool', 'bitmask', 'bcd'] as const) {
  ...
  test('bytes after the window are ignored — and the window itself is copied out', () => {
    ...
    payload[1] = 0x99;
    assert.equal(toHex(decoded?.raw ?? new Uint8Array()), '2A', 'the recorded raw bytes are the ones that were decoded');
  ...
  test('a bit window outside its container is refused instead of reading zeros', () => {
    ...
    try {
      ...
      assert.match((error as Error).message, /bit window 4\.\.12 does not fit the 8 bit container of DID 0x1234/);
    }
  });

  test('a bit window of zero or negative width is refused, a full-width one is accepted', () => {
    expect(lenient.decode(signal({ length: 1, bitOffset: 0, bitLength: 0, encoding: 'bitmask' }), fromHex('AB'))).toBeNull();
    expect(lenient.decode(signal({ length: 1, bitOffset: -1, bitLength: 4, encoding: 'bitmask' }), fromHex('AB'))).toBeNull();
    const full = lenient.decode(signal({ length: 1, bitOffset: 0, bitLength: 8, encoding: 'bitmask' }), fromHex('AB'));
  ...
  test('a truncated BCD nibble is reported, a partially numeric one is not silently truncated', () => {
    ...
    assert.equal(lenient.decode(bcd, fromHex('0A 34'))?.value, 0, 'the leading "0a" parses as 0 and swallows the rest');
    expect(lenient.decode(bcd, fromHex('FF FF')), 'nothing numeric in it at all').toBeNull();
    assert.equal(lenient.decode(bcd, fromHex('0A 34'))?.rawHex, '0A 34', 'the raw bytes are kept either way');
