      ...
      fc.assert(
        fc.property(
          ...
          (spec, k, scale, offset) => {
            ...
            const signal = baseSignal({ encoding: spec.encoding, length: spec.length, endianness, scale, offsetValue: offset });
...
describe('special encodings', () => {
  ...
  test('property: bitmask preserves the raw pattern and skips scaling by design', () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 255 }), fc.constantFrom('big', 'little' as const), (pattern, endianness) => {
  ...
  test('property: float32 round trips within float32 precision, both byte orders', () => {
    ...
    fc.assert(
      fc.property(
        fc.double({ min: -1_000_000, max: 1_000_000, noNaN: true, noDefaultInfinity: true }).filter((v) => !Object.is(v, -0) && Math.abs(v) > 1e-35),
...
describe('invalid input is rejected (strict) or reported (lenient)', () => {
  ...
  test('property: unsigned encodings reject negative values, signed accept them', () => {
    fc.assert(
      fc.property(fc.integer({ min: -32_768, max: -1 }), fc.constantFrom('big', 'little' as const), (negative, endianness) => {
  ...
  test('non-numeric input to a numeric signal is rejected with the signal id', () => {
    ...
    assert.throws(() => encodeValue(signal, 'not-a-number' as unknown as number), /engine\.property/);
  });
