...
describe('queries', () => {
  test('signalIds, samplesFor and the window filter', () => {
    ...
    recorder.record(decoded({ signalId: 'engine.coolant', name: 'Coolant', unit: '°C', value: 90, rawValue: 90, rawHex: '5A' }), 2000);
  ...
  test('anomalies flag out-of-range samples and extreme deltas', () => {
    ...
    recorder.record(decoded({ signalId: 'engine.coolant', name: 'Coolant', value: 250, rawValue: 250, outOfRange: true }), now + 50);
    recorder.record(decoded({ signalId: 'engine.coolant', name: 'Coolant', value: 90, rawValue: 90, outOfRange: false }), now + 60);
    const anomalies = recorder.anomalies();
    assert.ok(anomalies.some((entry) => entry.signal === 'engine.coolant' && entry.reason.includes('outside the declared range')));
  });
});
  ...
  test('property: a flat recording produces no anomalies regardless of length', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 60 }), (count) => {
        const recorder = new MeasurementRecorder(() => 1000);
        for (let i = 0; i < count; i++) recorder.record(decoded({ value: 2000, rawValue: 2000 }), 1000 + i);
