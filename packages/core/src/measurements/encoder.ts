  ...
  switch (signal.encoding) {
    case "ascii": {
      if (typeof value !== "string")
        throw new EncodeError(`signal ${signal.id} expects a string`, { signalId: signal.id });
    ...
    case "int32": {
      ...
      if (raw < 0 && signal.encoding.startsWith("uint")) {
        throw new EncodeError(
          `signal ${signal.id} is unsigned but the value ${value} encodes to ${raw}`,
          { signalId: signal.id },
        );
      }
      return writeUint(raw, signal.length, little);
...
function toNumber(signal: SignalDefinition, value: number | string | boolean): number {
  ...
  if (!Number.isFinite(parsed))
    throw new EncodeError(`signal ${signal.id} expects a numeric value, got "${value}"`, {
      signalId: signal.id,
    });
  return parsed;
}
