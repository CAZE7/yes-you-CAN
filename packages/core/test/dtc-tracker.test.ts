...
import { DtcTracker, DtcScanner } from '../src/index.js';
...
test('first and last seen are tracked per ECU and code (AGENTS 20)', () => {
  const scanner = scannerWithClock(['2026-01-01T10:00:00.000Z', '2026-01-01T10:05:00.000Z', '2026-01-01T10:10:00.000Z']);
  ...
  assert.equal(first[0]?.firstSeenInThisScan, true, 'the first scan that sees a code reports it as new');
  ...
  assert.equal(misfire?.firstSeenInThisScan, true, 'a code that appears later is new even though the ECU was scanned before');
  ...
  assert.deepEqual(
    scanner.tracker.snapshot().map((entry) => `${entry.ecuId}:${entry.code}:${entry.scans}`).sort(),
...
test('the same code on two ECUs is tracked separately', () => {
  ...
  assert.equal(other[0]?.firstSeenInThisScan, true, 'a shared code is a different occurrence per ECU');
