...
import { ChartGroup, Series, TimeViewport, computeYRange, decimate, decimateMinMax, formatClock, niceTicks, niceTimeTicks, type Point } from '../src/index.js';

function line(from: number, to: number, step: number, fn: (t: number) => number = (t) => t): Point[] {
  const points: Point[] = [];
...
test('series slice uses inclusive bounds and stays empty outside the data', () => {
  ...
  assert.deepEqual(series.slice({ from: 200, to: 300 }).map((p) => p.t), [200, 300]);
...
test('decimation output stays chronologically sorted and bounded', () => {
  ...
  for (const mode of ['minmax', 'lttb'] as const) {
    ...
    for (let i = 1; i < reduced.length; i++) {
      assert.ok((reduced[i]?.t ?? 0) >= (reduced[i - 1]?.t ?? 0), `${mode}: output must stay sorted`);
    }
...
test('fit shows the whole recording, follow keeps the span at the newest sample', () => {
  const viewport = new TimeViewport({ span: 1000, bounds: { from: 200, to: 1200 }, paddingFraction: 0.1 });
  viewport.fit();
...
test('selections are normalized and reported per series', () => {
  const group = new ChartGroup({ defaultSpanMs: 10_000, follow: false });
  group.push('engine.rpm', line(0, 5000, 100, (t) => t));
...
test('y range widens a flat line and honours declared limits', () => {
  ...
  assert.ok(limited.min <= 0 && limited.max >= 100, 'declared limits are always visible (AGENTS 14)');
