...
export class UdsNegativeResponseError extends VdpError {
  ...
  constructor(serviceId: number, nrc: number, nrcName: string, details: Record<string, unknown> = {}) {
    super('E_UDS_NEGATIVE_RESPONSE', `UDS negative response 0x${nrc.toString(16)} (${nrcName}) for service 0x${serviceId.toString(16)}`, {
      serviceId,
      nrc,
      nrcName,
      ...details,
...
export class SafetyViolationError extends VdpError {
  ...
  constructor(message: string, failedPreconditions: string[], details: Record<string, unknown> = {}) {
