...
describe('createId', () => {
  test('the default prefix is "id" and the shape is prefix_time_counter', () => {
    assert.match(createId(undefined, () => 0), /^id_0_[0-9a-z]+$/);
  });
  ...
  test('two ids from the same millisecond differ by the counter alone', () => {
    ...
    assert.equal(partsOf(second).counter - partsOf(first).counter, 1, 'the counter advances by one per id');
  ...
  test('ids are unique inside one clock tick and the counter advances by one', () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (previous !== undefined) {
        ...
        assert.equal((counter - previous + 0xffff) % 0xffff, 1, 'the counter advances by one per id');
      }
      previous = counter;
    }
  ...
  test('the counter rolls over at 0xffff — the documented bound of a frozen clock', () => {
    ...
    assert.equal(createId('wrap', () => 7), first, 'the counter restarts after a full cycle');
  });
...
describe('nowIso', () => {
  test('the injected clock decides the timestamp, in UTC with milliseconds', () => {
    assert.equal(nowIso(() => Date.UTC(2026, 0, 2, 3, 4, 5, 678)), '2026-01-02T03:04:05.678Z');
  });
