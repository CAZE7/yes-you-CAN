...
import { type Disposable, TypedEmitter } from "./events.js";
...
describe("TypedEmitter — lifecycle edge cases", () => {
  test("the same listener function is registered once, not twice", () => {
    ...
    emitter.on("frame", shared);
    assert.equal(
      emitter.listenerCount("frame"),
      1,
      "a Set stores an identical function once — no accidental double handling",
    );
  ...
  test("a listener subscribed during an emit joins the next one, not the running one", () => {
    ...
    assert.deepEqual(
      seen,
      ["first", "first", "late"],
      "the new listener is live for the next emit",
    );
  });
  ...
  test("a listener removed during an emit still runs in that emit", () => {
    ...
    assert.deepEqual(
      seen,
      ["remover", "removed", "remover"],
      "and it is gone from the next emit on",
    );
  });
    ...
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
            id: fc.nat({ max: 12 }),
            action: fc.constantFrom("on" as const, "off" as const, "dispose" as const),
          }),
          { maxLength: 80 },
        ),
        (ops: Op[]) => {
