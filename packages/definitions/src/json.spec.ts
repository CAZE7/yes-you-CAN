  ...
  assert.throws(
    () =>
      parseDefinitionPackage({
        ...
        ecus: [],
        signals: [{ id: 'x', name: 'x', ecu: 'e', did: 1, byteOffset: 0, length: 1 /* encoding missing */ }],
      }),
    (error: unknown) => {
    ...
    ecus: [
      {
        ...
        address: { txId: 0x7b0, rxId: 0x7b8, extended: true, addressing: 'extended', functionalId: 0x7df },
  ...
  assert.throws(
    () =>
      parseDefinitionPackage({
        ...
        signals: [{ id: 's', name: 'S', ecu: 'e', did: 'x', byteOffset: 0, length: 1, encoding: 42 }],
      }),
    (error: unknown) => {
      assert.ok(error instanceof DefinitionError);
