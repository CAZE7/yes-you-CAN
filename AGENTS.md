...
| Tests | ✅ 991 Tests auf 6 Ebenen (unit / protocol / regression / replay / integration / architecture) + 1 hardware smoke | Vitest 5 mit Projektkonfiguration (ADR 0010, Schritt 1); Unit-Specs co-lokatiert (`src/*.spec.ts`), Property-Tests (fast-check), Coverage-Gates global 90/80 als Durchschnitt (Ist 96,3/84,2), per-file laut `vitest.config.ts` (ADR 0017, angehoben durch ADR 0020) — grün; Gesamtlauf ~24 s, weil Discovery in Tests ein explizites Zeitbudget fährt und auf Bedingungen statt auf feste Sleeps gewartet wird (ADR 0019); `hardware` (`tests/hardware/vcan.test.ts`) läuft manual (`npm run test:hardware`), der nächtliche Job ist Teil von E10 |
...
npm run test:coverage # Suite + V8-Coverage — global 90/80/90/90 als Projekt-Durchschnitt,
                      ...
                      # (maßgeblich ist vitest.config.ts, ADR 0017/0020) — grün
...
| E11 | P2 | `packages/core/src/diagnostics/ecu-session.ts` liegt bei 85,6 % lines / 65,3 % branches und damit *auf* seinem Gate (`core` 85/65, per file). Jede kleine Änderung kann die Datei rot machen, ohne dass ein echter Mangel vorliegt; unbeobachtet bleiben u. a. die Zweige um `probeSupportedServices`, NRC-Behandlung und Timing-Übernahme aus der Session-Antwort (ISO 14229-2). | Fehler-/NRC-Zweige und die Session-Timing-Übernahme gezielt nachtesten, danach das `core`-Gate anheben (ADR 0017/0020: erst Tests, dann Gate). Keine neue Logik. |
| E12 | P2 | `packages/adapters/host/src/catalog.ts` ist mit 68,0 % lines / 48,6 % branches die dünnste Datei im Baum (Gate `adapters` 65/45). Sie probebt externe Kommandos (`ip`, `ls`, tty-Zugriff) und entscheidet damit, welcher Adapter dem Operator angeboten wird — unbeobachtete Zweige heißen hier „Adapter fälschlich als verfügbar/unverfügbar gemeldet“. | Die Probe-Pfade mit injizierten Runner-Funktionen nachtesten (verfügbar / nicht vorhanden / Fehlerausgang / unerwartete Ausgabe), danach das `adapters`-Gate anheben. `serial.ts` und `socketcan/binding.ts` bleiben als Hardware-Module ausgenommen (ADR 0016 §2). |
| E13 | P3 | `packages/storage/src/zip.ts` erreicht 100 % lines, aber nur 58,1 % branches; `migrations.ts` 95,5/61,5. Das `storage`-Gate (90/55) ist damit erfüllt, die Branch-Puffer sind jedoch dünn: ZIP-Export und Session-Migrationen sind der Weg, auf dem gespeicherte Diagnosedaten das Haus verlassen (AGENTS 14, Regel 34.13/34.14). | Fehlerzweige des ZIP-Readers (defekte/zerschnittene Archive, falsche Magics) und die Migrations-Pfade (unbekannte Version, Vorwärts-/Rückwärtskompatibilität) nachtesten; Gate anschließend auf 90/70 anheben. |
| E14 | P3 | Der Vitest-Runner meldet nach jedem Lauf: 75 Worker gestartet, ~107 ms Start je Datei, „at least ~7.89s faster with `isolate: false`“. Gemessen 2026-09-12 bei 23,83 s Gesamtlauf — Worker-Start ist damit ~33 % der Suite. | `isolate: false` (oder `pool: 'forks'` mit geteilten Workern) für das `unit`-Projekt messend erproben: dreimal grün in Folge, keine geteilten Moduleffekte (Logger-Sinks, feste Uhren), danach erst übernehmen. Determinismus schlägt Laufzeit (AGENTS 31) — bleibt der Nachweis aus, bleibt die Isolation. |
| E15 | P3 | `apps/web/src/backend.ts` ist mit 1238 Zeilen das größte Modul des Baums (nächste: `iso-tp/connection.ts` 721, `runtime/services.ts` 679). Es mischt Transportauswahl, Rohspur, Präsentation (`toEcuView`/`toDtcView`/`toTraceView`) und HTTP-Zustand — dieselbe Mischung, die ADR 0014 für die Engine aufgelöst hat. | In Kollaborateure teilen (Adapter-/Bus-Auswahl, View-Mapper, Präsentationszustand) ohne Verhaltensänderung; die View-Mapper sind der natürliche erste Schnitt, weil sie bereits als `*View`-Typen der Domain-Projektion gegenüberstehen (Roadmap-Schritt 9). Architekturtest-Allowlist bleibt unverändert. |
