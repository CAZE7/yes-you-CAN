...
> **Version:** 1.50 · **Letzte Änderung:** 2026-09-23
> **Changelog:**
> - 1.50: **Die Empfehlung aus Phase 0 ist umgesetzt: Apache-2.0 + CLA (ADR 0061/0062).** Aus der vorgelegten Entscheidungsvorlage wurden Rechtsakte im Baum. (1) **`LICENSE` trägt den vollständigen Apache-2.0-Text** (kanonische Fassung, 11 357 Zeichen inkl. Anhang — nicht gekürzt, nicht umformuliert; der Anhang-Platzhalter in der heute von apache.org ausgelieferten Schreibweise), **`NOTICE`** nennt Projekt, Copyright und die gemessene Tatsache, dass Fremdcode seine Hinweise bei sich trägt, **`TRADEMARK.md`** sagt, was die Lizenz **nicht** abdeckt (Apache-2.0 § 6 — Name und Marke bleiben draußen; beschreibende Verwendung und Fork-Kennzeichnung sind erlaubt). `"license"`: **30 Manifeste** (Root + 29 Pakete) auf `Apache-2.0`; README-Badge und `CONTRIBUTING.md` nachgezogen. **Was unter MIT verbreitet wurde, bleibt MIT** — die Umstellung gilt für neue Fassungen (Konzept §4.7). (2) **Beiträge laufen über eine Vereinbarung:** `docs/legal/cla.md` Fassung 1.0 (Individual + Corporate-Kurzform + Erfassung der Zustimmung), Register `docs/legal/contributors.md` (heute ehrlich leer: 1 Commit, 1 Autor, kein Fremdbeitrag). Der Vertrag räumt **Nutzungsrechte** ein — inklusive unbekannter Nutzungsarten (§ 31 Abs. 4 UrhG) und Sublizenzierung — und **überträgt kein Urheberrecht**, weil § 29 Abs. 1 UrhG das nicht zulässt; Patentlizenz mit Vergeltungsklausel gehört dazu, DCO (`git commit -s`) bleibt als Herkunftssignal, ist aber kein Ersatz. (3) **Das Gate:** `tests/architecture/legal.test.ts` (4 Tests) hält die Entscheidung fest — vollständiger Lizenztext inkl. §§ 1–9 und Anhang (der MIT-Grant-Satz darf nicht überleben), `NOTICE` vorhanden, Root **und** jeder Workspace-Manifest auf `Apache-2.0` (den Wert, nicht nur die Gleichheit), CLA versioniert und mit dem Satz `§ 29 Abs. 1 UrhG`/§ 31/Patentlizenz, Register + Verlinkung aus `CONTRIBUTING.md`/`TRADEMARK.md`. **Offen (0.E E28):** anwaltliche Prüfung des CLA, Signaturwerkzeug/Bot, Registry-Zugang. **Messung am Stand:** `npm run ci` EXIT 0 — `biome check .` 531 Dateien, `check:deps` 29/92/6, `check:manifests` agree, `check:api` 7/49/5, `check:licenses` 108/0/108, `npm test` **2543 Tests grün** (179 Dateien).
> - 1.49: **Phase 0 liegt zur Unterschrift vor — und der Trockenlauf hat vor der ersten Veröffentlichung einen echten Fund gemacht (0.E E28/E29).** Zwei Dokumente, keine Rechtsakte: `docs/architecture/open-core-phase-0-rights.md` legt die Entscheidungen **E1** (Lizenz des Kerns: MIT behalten · **Apache-2.0** (Empfehlung) · AGPL-3.0) und **E2** (Beitragsinstrument: **CLA als Nutzungsrechteeinräumung** (Empfehlung, weil § 29 Abs. 1 UrhG eine Übertragung nicht zulässt) · DCO · keines) mit Folgen, Kosten und Umsetzungs-Checkliste vor, samt Fragenliste für die Juristin und einem CLA-Entwurf in Anhang A; `docs/operations/registry.md` beschreibt Scope/2FA/OIDC/`--provenance` und die Reihenfolge der ersten Veröffentlichung. **Gemessen statt vermutet:** die Historie hat *einen* Commit mit *einem* Autor (Bot) — sie belegt also keine Urheberschaft, deshalb ist die Rechteklärung heute billig und ab dem ersten Fremdbeitrag nur noch mit Zustimmung aller möglich (Konzept §4.7). Der Trockenlauf (`npm pack --dry-run --json`) fand: `packages/domain` würde **20 Dateien, davon 17 `src/` inkl. Specs, und 0 `dist/`** ausliefern, weil kein Paket `files` deklariert und npm auf `.gitignore` (mit `dist/`) zurückfällt; `tools/harvest` 24/21/9/0. Behoben ist die **Regel**: `check-package-manifests.mjs` prüft für veröffentlichbare Pakete `files` (`publishable-without-files`, `publishable-ships-sources`, `publishable-without-dist` — die dritte liest die eigenen `main`/`types`/`exports`-Pfade), Biss-Fixtures in `manifests.test.ts` (20 → 22 Tests). Heute strukturell ruhig (29/29 privat) — genau deshalb steht die Regel vor der ersten `private: false`. **Messung am Stand:** `npm run ci` EXIT 0 — biome 529 Dateien, `check:deps` 29/92/6, `check:manifests` agree, `check:api` 7/49/5, `check:licenses` 108/0/108, `npm test` **2539 Tests grün** (178 Dateien).
> - 1.48: **Die Doku ist ein Gate, und die Verträge haben Seiten (ADR 0059, Phase 1.3 des Konzepts).** Zwei Dinge fehlten nach 1.47 noch: eine Seite je Naht, die ein Konsument *außerhalb* dieses Repos lesen kann, und eine Instanz, die toten Links hinterherräumt. **Neu:** `docs` in jedem `contracts`-Eintrag (Schema im `check-dependencies.mjs`, Datei-Existenz und Inhalt in `tests/architecture/docs.test.ts`) + sechs neue Naht-Seiten (`docs/api/{domain-ports,definitions-schema,oem-protocol,ai-provider,integrity-port,seed-key}.md`) und `docs/flows/open-core-boundary.md` als der eine Text, der die Grenze beschreibt (was hinüber darf, was nie — Autoritäts-Regel — und welches Gate was hält). **Der Link-Check ist Teil der Suite** (kein neues Werkzeug, ADR 0002/0010): `tests/architecture/docs.test.ts` prüft 133 Markdown-Dateien mit 293 relativen Links auf Existenz **und Anker** (GitHub-Slug), gleicht das ADR-Register gegen `docs/adr/` ab (59 ⇔ 59) und beweist seine Zähne an einem Fixture-Baum mit genau zwei Defekten — ein toter Link und ein verschobener Anker, während ein eigener Anker, eine externe URL und ein Beispiel im Codeblock nicht gemeldet werden. **Fund beim ersten Lauf:** zwei tote Links in `docs/standards/conformance.md` (ADR 0053/0054 ohne `../adr/`-Präfix) — im selben PR behoben, denn ein 404 auf ein ADR liest sich wie „den ADR gibt es nicht". **Messung am Stand:** `npm run ci` EXIT 0 — `biome check .` 529 Dateien, `check:deps` 29/92/6, `check:manifests` agree, `check:api` 7/49/5, `check:licenses` 108/0/108, `npm test` **2537 Tests grün** (178 Dateien, 5 neue in `docs.test.ts`).
> - 1.47: **Die Open/Closed-Grenze ist eine Messung statt einer Absicht (ADR 0059/0060).** Aus der Konzeptarbeit zu Open Core + Dual Licensing (`docs/architecture/open-core-dual-licensing.md`, Entwurf, nicht normativ) sind zwei Gates geworden, die vorher Prosa waren. (1) **`architecture/public-api.json` friert die Vertragsfläche ein:** `contracts` in `architecture/architecture.yaml` (sieben Verträge, jeder mit `why`; `entry` nur dort, wo der Vertrag ein *Modul* ist — `@vdp/core` → `logging/integrity`, `@vdp/protocols-uds` → `security`) wird von `npm run check:api` aus den emittierten `.d.ts` transitiv über `from "…"`-Spezifizierer aufgesammelt, kommentar- und whitespace-normalisiert gehasht und gegen den Record gehalten — gemessen **7 Verträge, 49 Flächen-Dateien, 5 externe Typquellen** (`@vdp/definitions`, `@vdp/diagnostic-ir`, `@vdp/protocols-uds`, `@vdp/shared`, `@vdp/transport-can`), EXIT 0; die Biss-Probe (eine Zeile an `packages/domain/dist/src/capabilities.d.ts`) fällt als `contract-drift` und nennt die Datei. `--update` schreibt den Record und **verweigert** bei defekter Konfiguration (EXIT 1, nichts geschrieben). (2) **`npm run check:licenses` prüft den Fremdcode** gegen `licenses` in `architecture.yaml` (zwei Geltungsbereiche, je mit `why`): gemessen **108 Drittpakete — 0 Produktion / 108 Entwicklung**, alle innerhalb der Richtlinie (die 12× MPL-2.0 `lightningcss*` liegen im Entwicklungsbereich, nicht in Produktion); unbekannte Lizenz, undatierte/abgelaufene/unbenutzte Ausnahme und ein `AND` mit verbotenem Glied fallen, `OR` darf sich auf einen erlaubten Zweig stützen. (3) **Die Verteilungsgrenze** (`private-dependency-leak` in `check:package-manifests.mjs`) verbietet einem veröffentlichbaren Paket jede private Abhängigkeit — heute strukturell ruhig (alle 29 Pakete sind `private`), bewiesen in `tests/architecture/manifests.test.ts`. Beide Gates hängen in `npm run ci` **hinter** `npm run build`; die sieben Vertragspakete tragen den Satz in ihren READMEs, der aus der Messung eine Regel macht. Messung am Stand: `npm test` **2531 Tests grün** (177 Dateien, 8 Skips), `biome check .` sauber, `check:deps` 29/92/6, `check:manifests` „29 packages checked“.
...
7. Führe `npm run check:deps`, `npm run check:manifests`, `npm run check:api`
   **und** `npm run check:licenses` aus — alle vier sind Teil von `npm run ci`
   und fallen bei einer Regelverletzung (ADR 0059/0060).
...
npm run check:api    # Vertragsfläche ⇔ architecture/public-api.json — *nach* `npm run build` (ADR 0059)
npm run check:licenses # Fremdcode-Lizenzen gegen die Richtlinie in architecture.yaml (ADR 0060)
...
| E28 | P1 | **Phase 0 ist entschieden und umgesetzt (Apache-2.0 + CLA), offen bleibt die anwaltliche Prüfung.** Gemessen 2026-09-23 vor der Umsetzung: Historie = **1 Commit, 1 Autor** (Bot), kein CLA/DCO im Baum, `LICENSE` = MIT. Umgesetzt im selben Tag: `LICENSE` (vollständiger Apache-2.0-Text, kanonisch), `NOTICE`, `TRADEMARK.md`, `"license": "Apache-2.0"` in 30 Manifesten, `docs/legal/cla.md` **Fassung 1.0** (Nutzungsrechte statt „Assignment“, § 29 Abs. 1 UrhG und § 31 Abs. 4 ausdrücklich adressiert, Patentlizenz mit Vergeltung, DCO zusätzlich), Register `docs/legal/contributors.md`, Gate `tests/architecture/legal.test.ts` (4 Tests), ADR 0061/0062, CODEOWNERS (Rechtsfragen nur Inhaber). | **Drei Dinge sind bewusst nicht erledigt:** (1) **anwaltliche Prüfung** des CLA-Textes und der OEM-Datenabgrenzung (Software vs. Datenbank, §§ 87a ff. UrhG) — Fragenliste in `docs/architecture/open-core-phase-0-rights.md` §6; (2) **Signaturwerkzeug/Bot** für Zustimmungen — bis dahin ist der wörtliche Satz aus `cla.md` im PR der Beleg, und `docs.test.ts`/`legal.test.ts` sichern, dass Text, Fassung und Verlinkung nicht verschwinden; (3) **Registry-Zugang** (Scope, 2FA, OIDC) aus `docs/operations/registry.md` §2 — Voraussetzung der ersten Veröffentlichung. Ohne (1) darf niemand den CLA als „geprüft“ bezeichnen (ADR 0062, AI implementation notes). || E29 | P2 | **Der Trockenlauf vor der ersten Veröffentlichung hat einen echten Fund ergeben — behoben ist die Regel, nicht der Fall (gemessen 2026-09-23).** `npm pack --dry-run --json` auf `packages/domain`: **20 Dateien, davon 17 `src/` (inkl. `*.spec.ts`), 0 `dist/`**; auf `tools/harvest`: 24 Dateien, 21 `src/`, 9 Specs, 0 `dist/`. Ursache: kein Paket deklariert `files`, npm fällt auf `.gitignore` zurück — und dort steht `dist/`, weil es der lokale Build-Ordner ist. Ein veröffentlichtes Paket hätte damit Quelltext samt Tests und keinen Build enthalten. Neu: `check-package-manifests.mjs` prüft für **veröffentlichbare** Pakete `files` (drei Regeln: `publishable-without-files`, `publishable-ships-sources`, `publishable-without-dist`, die dritte liest die eigenen `main`/`types`/`exports`-Pfade), Fixtures in `tests/architecture/manifests.test.ts` (22 Tests). Heute strukturell ruhig (29/29 privat). | Der erste Schritt der eigentlichen Veröffentlichung ist damit benannt und geprüft: `files: ["dist"]` und `private: false` je Vertragspaket in **einem** PR, `npm pack --dry-run` gegenlesen (nur `dist/**`, `package.json`, `README.md`, `LICENSE`), `--provenance`-Publish aus der CI — und erst danach Phase 3 (Konzept), [`docs/operations/registry.md`](docs/operations/registry.md) §3/§6. |
