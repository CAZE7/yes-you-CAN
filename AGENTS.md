...
> **Version:** 1.37 · **Letzte Änderung:** 2026-09-16
> **Changelog:**
...
| E24 | P2 | **Die Chaos-Schalter der Workbench zählen, was die Hülle sieht — die Session merken nichts.** `injectChaos()` legt seinen `CanChaosBus` *neben* den Bus, an dem die Runtime hängt: `apps/web/src/backend.ts:1039-1041` (`if (!this.chaosBus && this.bus) this.chaosBus = new CanChaosBus(this.bus)` — `this.bus` bleibt das Original, und `openBus()` (460-470) wickelt für Simulator- wie HF-Pfad gar nichts). Die Hülle abonniert den Innenbus und kriegt deshalb jeden Rahmen ab (`chaos-lab.ts:153-178`), nimmt der Sitzung aber keinen weg. Gemessen am Backend direkt (`npx vitest run --project integration apps/web/test/<Sonde>, ein Lauf, dann verworfen): `dropRate: 1` zählt 566 Rahmen in 600 ms, während dasselbe Backend `identify()` durchlässt, `scanDtcs()` 7 Codes liefert und die Polling bei 23 Signalen weiterläuft. Vor `start()` setzen geht auch nicht: `stop()` löscht `chaosBus`/`chaosDropRate`/`chaosDropBurst` (606, 613-614) und `start()` stoppt vorher — `chaosStatus()` meldet danach `active: false, dropRate: 0`. Zusätzlich zielt der Burst auf eine Adresse, die im Backend hart steht (1045, `0x7e0`); die `drop-count`-Regel isst nur Rahmen mit dieser Id (Semantik gepinnt in `tools/simulators/src/chaos-lab.spec.ts:124-133`), und `chaosStatus()` nennt das Ziel nicht. Deshalb hängt der Effekt am Fahrzeug: Demo-Fahrzeug 6 von 6 in ~600 ms, HF-Fahrzeug 0 von 6 nach 6 s (`dropBurstRemaining: 6`). | Chaos muss *in* den Pfad: `openBus()` wickelt den Session-Bus immer in `CanChaosBus` (ein Durchstich ohne eigenes Verhalten — die zwei Simulator-Zweige und der Hardware-Zweig), `injectChaos` verändert nur noch diese Instanz, `chaosStatus()` meldet die Ziel-Id des Bursts, und das Panel bekommt fürs Burst-Ziel dasselbe Feld, das die Korruptur schon hat (`#chaos-corrupt-can-id` in `apps/web/public/app.js`). Getestet wird die Wirkung an der Session, nicht am Zähler: bei Rate 1 muss `identify()` scheitern oder timeouten — sonst wirkt nichts. Die Frage „vor `start()` setzen“ ist neu zu entscheiden (verwerfen mit Grund und Kommentar, oder annehmen und wirksam machen), nicht still über ein Feld zu lösen. `apps/web/test/backend-paths.spec.ts` pinnt bis dahin ausschließlich, was die Schicht berichtet (Status, Reset, Zählerstand), und behauptet in keinem Wort eine Wirkung. |
