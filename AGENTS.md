...
> **Version:** 1.41 · **Letzte Änderung:** 2026-09-18
> **Changelog:**
> - 1.41: **Verbesserrunde ohne neues Feature: 69 neue Tests, zwei Lücken im Write-Port geschlossen, ein toter Export gestrichen, neun Gates nachgehoben — erst Tests, dann Gate (ADR 0017).** Ausgangspunkt war die E16-Methode (dünnste Datei je Gate-Gebiet nachmessen): 25 Grenz- und Misserfolgs-Tests in `charts` (Viewport-Guards, `Series`-Metadaten/`pushMany`/`statsOf`-Nicht-Zahlen, min/max-Reihenfolge bei fallenden Werten, LTTB/min-max-Entarten, Tick-Deckel, `ChartGroup`-Zoom-Limits, werfender Subscriber über den Default-Kanal, Readout-Farbe), in `core/writes` (Fehlerspeicher nicht lesbar, Nicht-Error-Rejections, verify ohne Permit, Adaptation/Coding auf Refusal-, Backup- und Rollback-Feuer), in `core/evidence` (Likelihood-Fallback, Id-Tie-Break, fehlender ecuId, `signalName === signal`), in `core/session` (fehlende Identitätsfelder, einseitiger Fault-Memory-Wandel, Nachkommastellen), in `ai` (Spread um Nullaverage, moderate Korrelationen, Ellipsis-Warnungen, namenloses Fahrzeug, wohlgeformte Gateway-Antwort unverändert durchgereicht) und in `signal-analysis` (moderate Bänder, simultane Zeitstempel, Einzelwert-FFT, sanfte Steigung). **Zwei Funde, beide behoben:** (1) `WritePort.run` verlor das `unproven`-Feld auf dem Weg vom Confirm-Stage zum Ergebnis — `StageReport` trägt nur reasons/warnings/detail, das Casten aus dem Report war immer leer; `unproven` fährt jetzt über eine lokale Variable, `run` und `precheck` liefern dieselbe Unterscheidung (P0 #5), ein Test pinnt sie. (2) `LIKELIHOOD_PRIOR` mappte `plausible`, das Schema kennt aber nur `common | possible | rare` — jedes schema-gültige `"possible"` fiel still auf den Unknown-Prior; Tabelle und Doku-Kommentar korrigiert, End-to-End-Test für den 0,45-Prior. Toter Export `TimeViewport.includes` gestrichen (kein Aufrufer im Baum, kein Doku-Beleg — Präzedenz `summarizeAvailability`). Gates gehoben, jedes mit gemessener Marge in `vitest.config.ts`: core 88/80 → **92/82**, protocols 90/75 → **93/78**, adapters-Zeilen 92 → **93**, transport 88/72 → **90/74**, reports-Zweige 80 → **82**, ai 95/85 → **95/88**, definitions 85/80 → **88/81**, charts 90/75 → **95/85**, diagnostic-ir 95/85 → **95/90**; zwischendurch maßen `dtc-clear.ts`, `adaptation.ts`, `encoder.ts`, `heuristic.ts`, `engine.ts`, `group.ts`, `scale.ts` **100/100** und `compare.ts` 100 Zeilen. README-Badge/Tests-Zeile/Ist-Zahlen und 0.A nachgezogen (2116 Tests / 143 Dateien, 62 s / 77 s, **95,67 / 88,81 / 96,61 / 96,75**). `npm run ci` EXIT 0 (siehe 0.A).
> - 1.40: **Der Core ist wieder portabel: SHA-256 ohne `node:crypto`, und die Doku sagt wieder, was gemessen ist.** `npm run check:deps` fiel mit `@vdp/core imports node:crypto, but is a portable layer` (`logging/integrity.ts`) — die Regel stand, der Import brach sie. Neu `packages/shared/src/sha256.ts` (FIPS 180-4, abhängigkeitsfrei, ohne `TextEncoder`/`Buffer`; `as const`-Tabellen und `as number`-Lesarten nach `bytes.ts`-Vorbild, damit kein toter Fallback-Arm Coverage kostet), `integrity.ts` hasht den längenpräfigierten Strom darüber — bitidentische Digests zum alten Pfad (nachgerechnet mit `node:crypto`, Manifeste bleiben gültig). Spec mit den fünf FIPS-Vektoren (leerer String, `abc`, 56-Byte, 112-Byte, 1 Mio. `a`) plus fast-check-Kreuzprüfung gegen `node:crypto` als Orakel (nur im Test — der Checker sieht Specs nicht); `sha256.ts` 100/100/100/100. **Das Core-Gate hat dabei gebissen, wie es soll:** `.map()` statt `for` nahm zwei gedeckte Zweige aus dem Nenner, `integrity.ts` fiel auf 77,77 Zweige — zwei neue Tests (Extended-/FD-Rahmen, `withRawTraceManifest`, das nie aufgerufen wurde) heben die Datei auf 100/100/100/100, ein dritter pinnt den Digest gegen den alten `node:crypto`-Wert als Stabilitätsanker, erst Tests, dann grün (ADR 0017). Dazu drei Biome-Formatfehler weg (`--write`, keine Handänderung) und Doku-Drift zurückgemessen: README-Badge und 0.A auf **2047 Tests / 143 Dateien** (`npm test` 63 s, Coverage 72 s mit **95,2 / 87,3 / 96,53 / 96,46**), `shared`-README kennt `sha256.ts` (und `bytes-ascii.ts` nicht mehr, das nie existierte — die Spec importiert aus `bytes.js`), ARCHITECTURE.md zählt **43 ADRs**. `npm run ci` EXIT 0.
