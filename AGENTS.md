...
> **Version:** 1.40 · **Letzte Änderung:** 2026-09-18
> **Changelog:**
> - 1.40: **Der Roh-Trace-Digest zieht in die Foundation: der Baum ist wieder grün, das Manifest hat seinen Known-Answer-Pin (ADR 0044).** Gefunden am Stand `fabc0aa`, gemessen nach `npm ci && npm run build`: `packages/core/src/logging/integrity.ts` hashte das Roh-Trace-Manifest mit `node:crypto`, und der Core ist eine portable Schicht — `npm run check:deps` meldete `✗ [node-builtin] @vdp/core imports node:crypto, but is a portable layer` (Exit 1), `dependencies.test.ts` scheiterte auf Modulebene und ließ **11 seiner Tests aus der Zählung verschwinden** (`npm test`: 2027 passed | 1 failed | 1 skipped, wo 2037 dokumentiert standen), `guardrails.test.ts` fiel über `biome check .` (3 Fundstellen: `integrity.spec.ts`, `session-logger.ts`, `scenarios/alternator_failure.json`), und der Dateikopf versprach einen Digest „in Node, a replay worker, and an export pipeline", den es mit Node-Binding im Worker nie gegeben hätte. Gebaut: `sha256Hex` in `@vdp/shared` (FIPS 180-4 §4.2.2/§5.3.3/§6.2, 60 Zeilen, keine Abhängigkeit, `mayImport: []` bleibt), `integrity.ts` ruft nur noch das Primitiv — **Allowlist nicht erweitert, Regel nicht gebeugt** (0.D). **Digest byte-identisch**, gemessen an 200 000 Rahmen (der Recorder-Obergrenze, 8,7 MiB kanonischer Satzstrom): `node:crypto` Satz für Satz, `node:crypto` über den zusammengesetzten Strom und `sha256Hex` darüber — dieselben drei Werte; als Konstante gepinnt in `integrity.spec.ts`, und zwar **von außen gemessen**, weil ein Erwartungswert aus demselben Code nichts behauptet. Preis: 135 ms statt 69,3 ms je Manifest, einmal pro Export, nicht pro Rahmen. 9 neue Tests (6 in `hash.spec.ts`: FIPS-Vektoren, jede Länge 0…200 gegen `node:crypto`, 100 KiB über 1600 Blöcke mit wiederverwendetem Scratch-Puffer, UTF-8 mit `ü`/`°`/Emoji, String gegen Bytes, `"ab"` gegen `"ab\0"`; 3 in `integrity.spec.ts`: Known-Answer, FD/Extended-Kanal, die vier Manifestfelder als Misstrauensfälle) — die beiden letzten sind keine Füller: nach dem Digest-Tausch maß `integrity.ts` 88,88 / 77,77 und **fiel durch das core-Tor 88/80**, jetzt 100/100/100/100 (`hash.ts` ebenfalls, `packages/shared` bleibt bei 100/100/100/100). `withRawTraceManifest` ist gestrichen: Export ohne Aufrufer *und* am echten Pfad vorbeitypiert (`SessionLogger.toJson` schreibt `payload` als Hex-String, der Helfer verlangt `RawTraceEntry[]`) — die offene Frage daraus steht als **E25** in 0.E: niemand liest das Manifest, gespeichert wird es nirgends. Demo am lebenden Objekt (931 Rahmen über `/api/start`, `/api/live/start`, `/api/dtc/scan`, Export `session.json` 362 233 Bytes, `report.pdf` unverändert 0 Fragezeichen): der aus dem Export gerechnete portable Digest = `node:crypto` über denselben Satzstrom = `node:crypto` Rahmen für Rahmen, alle drei `0b565a85…a0347c`; das Exportfeld selbst bleibt leer (E25). Messung am Stand dieses Commits: `npm run ci` **EXIT 0** (143 Dateien / 2048 Tests in 55,0 s bzw. 56,5 s — zwei Läufe), `npm run test:coverage` EXIT 0 in 62,4 s mit 95,19 / 87,29 / 96,52 / 96,45, `npx biome check .` 435 Dateien ohne Befund, `check:deps` „27 packages placed, 79 edges, 6 rules — no violations".
...
| Core (VIN, ECU-Discovery, DTC, Live-Engine, Recorder, Safety) | ✅ | DTC-System komplett: Freeze Frames, First/Last-Seen, Safety-gated Clear über den Write-Port (AGENTS 20/25/26, ADR 0032), Beobachtungen mit Beleg und **eine** Identitätsregel `dtcKey` (ADR 0037); Discovery ignoriert eigene tx-Echos (Regressionskatalog); Identifikationswerte tragen die DID, aus der sie gelesen wurden (§11/§12); Roh-Trace-Manifest (SHA-256 über den kanonischen Satzstrom) hat seinen Digest als Primitiv in `@vdp/shared`, der Core bleibt frei von `node:`-Builtins (ADR 0044) |
...
## 0.E Offene Verbesserungen — Backlog (Stand 2026-09-18)
...
| E25 | P2 | **Das Roh-Trace-Manifest hat ein Zeugnis ohne Leser.** `packages/core/src/logging/integrity.ts` erzeugt den Digest (`sha256` über den kanonischen Satzstrom, Primitiv jetzt in `@vdp/shared` — ADR 0044), und `SessionLogger.rawTraceManifest()` liefert ihn. Gemessen 2026-09-18: `grep -rn "rawTraceManifest" packages apps tools --include=*.ts` ohne `packages/core/src/logging` → **kein Treffer**; der einzige Aufrufer der Methode ist die Methode nicht selbst, sondern niemand. Kein Export trägt das Feld: `/api/export/session.json`, `/api/export/trace.csv`, `/api/export/*` in `apps/web/src/server.ts` und `packages/storage/src/zip.ts` schreiben Trace-Rohdaten, aber keinen Digest — `tools/golden-sessions` ebenso nicht. Der Fall, den das Manifest schützt (eine Aufnahme, die nachträglich umgeschrieben wurde), ist damit unbeweist, obwohl der Beweiser im Baum liegt. Der gestrichene Helfer `withRawTraceManifest` (ADR 0044) hätte das auch nicht geheilt: er verlangt `{ trace: RawTraceEntry[] }`, der echte Export schreibt `payload` als Hex-String. | **Entscheidung, nicht Nachtesten:** das Manifest in den Export legen — additiv in das Sitzungs-JSON (`format: "vdp.session"`, `formatVersion` bleibt 1, kein Schema-Bump: addive optionale Felder brauchen ihn nicht, gemessen und begründet in ADR 0026) und beim **Lesen** mit `verifyRawTraceManifest` prüfen, damit ein abgeschnittener Export ein benannter Defekt ist statt Stille. Oder der Modulzweck ist erfüllt und die drei öffentlichen Funktionen bleiben, wie sie sind, als Vertrag für den Export-Pfad — dann aber mit diesem Eintrag als Grund, warum niemand sie sucht. Was es **nicht** ist: eine Datei neben `session.json` (AGENTS §23: ein Zeugnis liegt bei dem, was es beglaubigt) und kein Reader ohne Writer. |
