...
> **Version:** 1.36 · **Letzte Änderung:** 2026-09-16
> **Changelog:**
...
| E23 | P3 | **Ein Tipp im Feld „ECU“ zählt als Serverfehler.** `POST /api/dtc/snapshot` mit einer Adresse, die niemand auf dem Bus hat, antwortet `500` mit dem Satz `unknown ECU "0x7f0" — connect first or check the id` und schreibt eine `request failed`-ERROR-Zeile ins Log (gemessen am laufenden Demo-Server, als Ist-Zustand gepinnt in `apps/web/test/server-paths.spec.ts`). Die Aussage stimmt, die Klasse nicht: das ist ein Vorzustand, den der Operator heilen kann — ADR 0018 behandelt genau das als Ablehnung mit Daten („a refused clear is an answer with reasons, not a server error“). | Entweder löst `ecuRef` an der Grenze in einen `HttpError(409, …)` auf, oder die Runtime meldet die Klasse (`UnknownEcuError`) und das HTTP-Layer ordnet zu; dann diese Zeile und die 400er-Absagen im selben Diff auf 409 drehen und den Pin nachziehen. Nicht der umgekehrte Weg: 500er aus dem Log verschwinden lassen, damit die Zahl schöner aussieht. |
