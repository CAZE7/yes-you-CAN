...
> **Version:** 1.49 · **Letzte Änderung:** 2026-09-23
> **Changelog:**
...
| E28 | P1 | **Phase 0 des Open-Core-Konzepts ist vorgelegt, aber nicht entschieden — und die Entscheidung wird mit jedem Fremdbeitrag teurer (gemessen 2026-09-23).** Gemessen: die Historie hat **einen** Commit mit **einem** Autor (`arena-ai-coding-agent[bot]`), also belegt sie *keine* Urheberschaft; es gibt **kein** CLA und **kein** DCO (kein `Signed-off-by` in `CONTRIBUTING.md` oder `.github/`); `LICENSE` ist MIT mit der Zeile „Copyright (c) 2026 yes-you-CAN Contributors". Damit ist heute jede Umstellung ohne Fremdzustimmung möglich — nach dem ersten angenommenen externen PR nur noch mit Zustimmung aller Rechteinhaber (oder per Clean Room). Vorgelegt: `docs/architecture/open-core-phase-0-rights.md` (Empfehlung **Apache-2.0** statt MIT wegen Patentgrant/Marke, Beitragsinstrument als **Nutzungsrechteeinräumung** statt „Assignment" — § 29 Abs. 1 UrhG lässt eine Übertragung nicht zu —, CLA-Entwurf in Anhang A, Fragenliste für die Juristin, Umsetzungs-Checkliste in Anhang B) und `docs/operations/registry.md` (Scope/2FA/OIDC/provenance). | Zwei Unterschriften (Lizenz, Beitragsinstrument), dann **ein** PR je Entscheidung: `LICENSE`/`NOTICE` + `"license"` in Root und 29 Paketen, `CONTRIBUTING.md` + `docs/legal/` + CLA-Bot, ADR 0061/0062, Konzept-Phase-0-Zeilen und diese Zeile schließen. Ohne Entscheidung bleibt der Baum offen für einen Schritt, der sich nicht zurücknehmen lässt (Konzept §4.7). |
| E29 | P2 | **Der Trockenlauf vor der ersten Veröffentlichung hat einen echten Fund ergeben — behoben ist die Regel, nicht der Fall (gemessen 2026-09-23).** `npm pack --dry-run --json` auf `packages/domain`: **20 Dateien, davon 17 `src/` (inkl. `*.spec.ts`), 0 `dist/`**; auf `tools/harvest`: 24 Dateien, 21 `src/`, 9 Specs, 0 `dist/`. Ursache: kein Paket deklariert `files`, npm fällt auf `.gitignore` zurück — und dort steht `dist/`, weil es der lokale Build-Ordner ist. Ein veröffentlichtes Paket hätte damit Quelltext samt Tests und keinen Build enthalten. Neu: `check-package-manifests.mjs` prüft für **veröffentlichbare** Pakete `files` (drei Regeln: `publishable-without-files`, `publishable-ships-sources`, `publishable-without-dist`, die dritte liest die eigenen `main`/`types`/`exports`-Pfade), Fixtures in `tests/architecture/manifests.test.ts` (22 Tests). Heute strukturell ruhig (29/29 privat). | Der erste Schritt der eigentlichen Veröffentlichung ist damit benannt und geprüft: `files: ["dist"]` und `private: false` je Vertragspaket in **einem** PR, `npm pack --dry-run` gegenlesen (nur `dist/**`, `package.json`, `README.md`, `LICENSE`), `--provenance`-Publish aus der CI — und erst danach Phase 3 (Konzept), [`docs/operations/registry.md`](docs/operations/registry.md) §3/§6. |
