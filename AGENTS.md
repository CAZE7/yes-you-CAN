...
> **Version:** 1.13 · **Letzte Änderung:** 2026-09-14
> **Changelog:**
...
| Tests | ✅ 1307 Tests auf 6 Ebenen (unit / protocol / regression / replay / integration / architecture) + 1 hardware smoke | Vitest 5 mit Projektkonfiguration (ADR 0010, Schritt 1); Unit-Specs co-lokatiert (`src/*.spec.ts`), Property-Tests (fast-check), Coverage-Gates global 90/80/90/90 als Durchschnitt (Ist 96,48 Statements / 89,65 Zweige / 97,52 Funktionen / 97,86 Zeilen), per-file laut `vitest.config.ts` für shared/core/protocols/adapters/transport/storage/charts/reports/ai/definitions (ADR 0017, angehoben durch ADR 0020 und 0022, `definitions` 85/80 neu durch ADR 0023); Struktur ist mitgetestet — Abhängigkeitsgraph, Hygiene-Regeln und die Manifest-Metadaten aller 25 Pakete (license/engines/repository.directory, `tests/architecture/manifests.test.ts`) — grün; Gesamtlauf 22,6 s (gemessen 2026-09-14; `npm run ci` 25,6 s), weil Discovery in Tests ein explizites Zeitbudget fährt und auf Bedingungen statt auf feste Sleeps gewartet wird (ADR 0019); `hardware` (`tests/hardware/vcan.test.ts`) läuft manual (`npm run test:hardware`), der nächtliche Job ist Teil von E10. Zähl-Falle beim Vergleichen von Zahlen: ein bloßes `npx vitest run` ohne `--project` nimmt `hardware` mit und meldet 92 Dateien / 1308 Tests statt 91 / 1307 — ohne `vcan0` besteht der Test per Selbst-Skip immer. |
| CI/CD | 🚧 GitHub Actions: `ci.yml` mit `npm ci` → `build` → `npm test` auf Node 22 + 24 (`checkout@v4`/`setup-node@v4`, Concurrency, `contents: read`) + Dependabot (gruppiert) | **Ist-Zustand nach Regel 34.24:** Quality-Job (`biome check`·`typecheck`·`npm audit`), Coverage-Upload, `codeql.yml`, `dependency-review.yml` und `hardware.yml` sind nach ADR 0016 §3 fertig entwickelt, liegen aber nur in der Arbeitskopie — GitHub lehnt den Push von Workflow-Dateien ohne `workflows`-Berechtigung der App ab (gemessen 2026-09-12). Verbindliches Tor ist deshalb `npm run ci`; Freischaltung und Folge-PR siehe 0.E E10. **Seit 2026-09-14 (ADR 0026) führt der `architecture`-Projektlauf `biome check .` und beide `--noEmit`-Pässe selbst aus** — die CI erzwingt die Quality-Gates damit ohne Workflow-Recht; die Gleichwertigkeit hängt an `tests/architecture/guardrails.test.ts` (0.E E17) |
...
| E18 | P1 | **`exactOptionalPropertyTypes` ist der größte offene Strictness-Gewinn — und eine echte Aufgabe.** Gemessen am 2026-09-14 mit `exactOptionalPropertyTypes: true`: **27 Fehler in 13 Produktionsdateien** im Build (TS2412 ×17, TS2375 ×5, TS2379 ×5) plus **61 Fehler in 28 Dateien** im Typecheck-Projekt (TS2412 ×30, TS2379 ×21, TS2375 ×8) — 88 insgesamt, konzentriert in den Parsern, die optionale Felder kopieren (`definitions/src/json.ts`, `runtime/src/mappers.ts`, `apps/web/src/backend.ts`). Der Grund ist nicht Konfigurationsaufwand: mit dem Flag muss jede Kopie eines optionalen Feldes sagen, ob sie `undefined` **oder** *fehlend* meint — und genau diese Verwechslung ist an der Roh→dekodiert-Grenze (ADR 0004) ein stiller Diagnosefehler. Das Flag steht deshalb mit dieser Messung in `RELAXED_FLAGS` (`tests/architecture/guardrails.test.ts`), nicht mehr als stille Abschwächung. | In Scheiben migrieren, mit den Datenschicht-Parsern zuerst: (1) `definitions` (Validator/Parser, dort ist „fehlt“ fachlich bedeutsam: Schema-v1-Bestand ohne `vehicles[]`), (2) `runtime`-Mapper, (3) `apps/web`-Zustand. Jede Scheibe mit Tests, dann das Flag global setzen und den Eintrag aus `RELAXED_FLAGS` streichen (Regel 34.21: erst messen, dann umstellen). Keine Semantikänderung an gespeicherten Sitzungen. |
| E19 | P3 | **Frontend-`noImplicitAny` bleibt abgeschaltet — mit Zahl statt Gefühl.** Gemessen am 2026-09-14 mit `noImplicitAny: true` in `tsconfig.frontend.json`: **110 Fehler in 4 Dateien** (`apps/web/public/*.js`, davon 104 × TS7006 implizite Parametertypen, 6 × TS2339). Das Frontend ist geprüftes JavaScript ohne JSDoc-Typen (`checkJs`), und die Schicht ist Rendering über bereits typisierten View-Projektionen (Roadmap-Schritt 9); das Tor, das zählt — kein `any` in Produktionscode — ist im TypeScript-Baum als `error` scharf. Der Wert steht mit dieser Messung in `RELAXED_FLAGS`. | Wenn das Frontend ohnehin angefasst wird: JSDoc-Typen für die DOM-Helfer und Event-Handler nachtragen (die 104 TS7006 sind fast alle Parameter von `addEventListener`-Rückrufen und Helfern wie `el(selector, tag, children)`), danach das Flag setzen und den Eintrag streichen. Kein eigener PR nur dafür — die Reihenfolge im Master-Backlog (#19/#27) ist bewusst nachrangig. |
