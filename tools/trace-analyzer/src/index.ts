...
export interface TraceFinding {
  kind: 'unknown-id' | 'malformed' | 'incomplete-transport-message' | 'negative-response' | 'high-rate' | 'info';
...
export function parseTraceLine(line: string, channel = 'trace'): RawTraceLine | null {
  ...
  if (trimmed.startsWith('{')) {
    try {
      const parsed = JSON.parse(trimmed) as { t?: number; canId?: number; extended?: boolean; dlc?: number; data?: number[] | string };
      if (parsed.canId === undefined || parsed.data === undefined) return null;
      const payload = typeof parsed.data === 'string' ? hexToBytes(parsed.data) : new Uint8Array(parsed.data);
      return {
        t: parsed.t ?? null,
        frame: { timestamp: parsed.t ?? 0, id: parsed.canId, extended: parsed.extended ?? false, fd: false, dlc: parsed.dlc ?? payload.length, payload, channel },
      };
    } catch {
      return null;
    }
  ...
  const candump = /^\s*\S+\s+([0-9A-Fa-f]{3,8})\s+\[(\d)\]\s+((?:[0-9A-Fa-f]{2}\s*)*)$/i.exec(trimmed);
  if (candump) {
...
export function decodeUdsRequest(payload: Uint8Array): { serviceId: number; did?: number } | null {
  ...
  if ((payload[1] !== undefined && (sid === 0x22 || sid === 0x2e)) || sid === 0x22 || sid === 0x2e) {
    const did = ((payload[1] ?? 0) << 8) | (payload[2] ?? 0);
  ...
  for (const message of messages) {
    ...
    if (response.positive && response.serviceId === 0x22 && response.did !== undefined && context.signalIndex && response.data) {
      ...
      for (const signal of candidates) {
        ...
        if (decoded) {
          decodedSignals.push({ t: message.firstFrameT, signal: signal.id, value: decoded.value, ...(decoded.unit ? { unit: decoded.unit } : {}) });
        }
      }
    }
...
export function rebuildIsoTpMessages(
  ...
  const idToPair = new Map<number, { txId: number; rxId: number; direction: 'request' | 'response' }>();
  ...
  const active = new Map<number, { parts: Uint8Array[]; expected: number; firstT: number; lastT: number; frames: number }>();
  ...
  for (const entry of entries) {
    ...
    if (pci === 0) {
      ...
      active.set(entry.frame.id, { parts: [data.subarray(2)], expected, firstT: entry.t, lastT: entry.t, frames: 1 });
    } else if (pci === 2) {
...
export function derivePairs(pkg: DefinitionPackage | undefined, entries: readonly TraceEntry[]): Array<{ txId: number; rxId: number }> {
  if (pkg) {
...
function decodeSignalAt(signal: SignalDefinition, data: Uint8Array): { value: number; unit?: string } | null {
  let raw = 0;
  ...
  return { value: raw * (signal.scale ?? 1) + (signal.offsetValue ?? 0), ...(signal.unit ? { unit: signal.unit } : {}) };
}
...
export function formatTraceReport(analysis: TraceAnalysis): string {
  ...
  for (const id of analysis.ids.slice(0, 40)) {
    const dlcs = Object.entries(id.dlcDistribution)
      ...
      .join(' ');
    lines.push(`| 0x${id.canId.toString(16).toUpperCase()}${id.name ? ` (${id.name})` : ''} | ${id.frames} | ${id.periodMs ?? '—'} ms | ${dlcs} | ${id.known ? 'yes' : 'no'} |`);
  }
  lines.push('', '## Decoded messages', '');
  for (const message of analysis.messages.slice(0, 40)) {
    lines.push(`- t=${message.firstFrameT} 0x${message.sourceId.toString(16)} ${message.direction} ${message.decoded ?? 'raw'}: ${message.payload}`);
  }
  lines.push('', '## Findings', '');
