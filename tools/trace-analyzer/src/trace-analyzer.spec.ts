...
test("candump, CSV and NDJSON trace lines all parse to frames", () => {
  ...
  assert.equal(
    candump?.frame.channel,
    "vcan0".replace("vcan0", "trace"),
    "channel defaults when the line carries none of its own",
  );
...
test("a full multi-frame exchange is rebuilt into one message", () => {
  ...
  const vin = analysis.messages.find(
    (message) => message.did === 0xf190 && message.direction === "response",
  );
...
test("negative responses and unknown identifiers are flagged", () => {
  ...
  const entries = parseTrace(
    ["  vcan0  7E8   [4]  03 7F 22 31", "  vcan0  123   [8]  00 00 00 00 00 00 00 00"].join("\n"),
  );
  const analysis = analyzeTrace(entries, { definitions: genericPackage });
  assert.ok(analysis.findings.some((finding) => finding.kind === "negative-response"));
  assert.ok(
    analysis.findings.some((finding) => finding.kind === "unknown-id" && finding.canId === 0x123),
  );
});
