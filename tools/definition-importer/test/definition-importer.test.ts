...
import { importCsv, importDbc, importJson } from '../src/index.js';

const PROVENANCE: Provenance = { sourceType: 'licensed', source: 'test fixture DBC, no OEM documentation used', license: 'test fixture license' };
...
test('DBC messages become ECUs and signals keep factor, offset and unit', () => {
  ...
  assert.equal(result.pkg.ecus[0]?.address.txId, 2024 - 8, 'ISO 15765-4: request id is the response id minus 8');
...
test('signals longer than the schema supports are reported, not truncated', () => {
  ...
  const result = importDbc(withLongSignal, { oem: 'test', name: 'long signal', provenance: PROVENANCE });
  assert.deepEqual(result.errors, []);
  assert.ok(result.skipped.some((entry) => /bitLength 136 exceeds/.test(entry.reason)), result.skipped.map((entry) => entry.reason).join(' | '));
});
...
test('unsupported DBC records are reported, not silently dropped', () => {
  ...
  assert.ok(result.skipped.some((entry) => entry.reason === 'unsupported DBC record' && entry.text.includes('SOME_UNKNOWN_RECORD')));
});
...
test('DBC imports carry their provenance into the package (AGENTS 24)', () => {
  ...
  assert.ok(!result.warnings.some((warning) => warning.includes('placeholder')), 'a documented provenance must not warn about placeholders');
});
...
test('CSV imports build ECUs from the txId/rxId columns', () => {
  ...
  const result = importCsv(csv, { oem: 'test', name: 'CSV import', provenance: { sourceType: 'community', source: 'test fixture' } });
  assert.deepEqual(result.errors, []);
...
test('CSV rows with an unknown encoding are skipped with a reason', () => {
  const csv = ['ecu,txId,rxId,did,byteOffset,length,encoding,name,unit,scale,offset', 'Engine,7E0,7E8,F40C,0,2,quantum,Engine speed,rpm,,'].join('\n');
  const result = importCsv(csv, { oem: 'test', name: 'bad encoding', provenance: { sourceType: 'community', source: 'test fixture' } });
...
test('a CSV without the required columns fails with an actionable error', () => {
  assert.throws(
    () => importCsv('name,unit\nfoo,bar\n', { oem: 'test', name: 'x', provenance: { sourceType: 'community', source: 'y' } }),
    (error: unknown) => {
...
test('JSON imports validate the package they produce', () => {
  const json = JSON.stringify({
    ecus: [{ id: 'engine', name: 'Engine', address: { txId: 0x7e0, rxId: 0x7e8 }, protocol: 'uds', services: [0x22] }],
    signals: [{ id: 'engine.rpm', name: 'Engine speed', ecu: 'engine', did: 0xf40c, byteOffset: 0, length: 2, encoding: 'uint16', scale: 0.25, unit: 'rpm' }],
  });
  const result = importJson(json, { oem: 'test', name: 'JSON import', provenance: { sourceType: 'own', source: 'written for this test' } });
  assert.deepEqual(result.errors, []);
  assert.equal(result.pkg.signals[0]?.id, 'engine.rpm');
});

test('invalid JSON is reported as a definition error', () => {
  assert.throws(() => importJson('{ not json', { oem: 'test', name: 'x', provenance: { sourceType: 'own', source: 'y' } }), /not valid/);
});
...
test('a reverse-engineered import still carries its warning (AGENTS 24)', () => {
  ...
  const result = importJson(json, { oem: 'test', name: 'no provenance', provenance: { sourceType: 'reverse-engineered', source: 'unknown' } });
  assert.ok(result.warnings.some((warning) => warning.includes('reverse-engineered')), `warnings were: ${result.warnings.join(' | ')}`);
});

test('a structurally broken import is rejected with the validator errors', () => {
  const result = importJson(JSON.stringify({ ecus: [], signals: [] }), { oem: '', name: 'broken', version: 'not-semver', provenance: { sourceType: 'own', source: 'y' } });
